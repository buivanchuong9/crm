...
*/
var require_react_development = /* @__PURE__ */ __commonJSMin(((exports) => {
	(function() {
		"use strict";
...
current: null };
		...
		*/
		function cloneElement(element, config, children) {
			if (!!(element === null || element === void 0)) throw Error("React.cloneElement(...): The argument must be a React element, but you passed " + element + ".");
			var propName;
		...
		*/
		function getElementKey(element, index) {
			if (typeof element === "object" && element !== null && element.key != null) return escape("" + element.key);
		...
		function mapIntoArray(children, array, escapedPrefix, nameSoFar, callback) {
			...
			if (invokeCallback) {
				...
				if (Array.isArray(mappedChild)) {
					...
					if (isValidElement(mappedChild)) mappedChild = cloneAndReplaceKey(mappedChild, escapedPrefix + (mappedChild.key && (!_child || _child.key !== mappedChild.key) ? escapeUserProvidedKey("" + mappedChild.key) + "/" : "") + childKey);
		...
		*/
		...
		function createContext(defaultValue, calculateChangedBits) {
			if (calculateChangedBits === void 0) calculateChangedBits = null;
			...
			if (payload._status === Uninitialized) {
				...
				thenable.then(function(moduleObject) {
					if (payload._status === Pending) {
						...
						if (defaultExport === void 0) error("lazy: Expected the result of a dynamic import() call. Instead received: %s\n\nYour code should look like: \n  const MyComponent = lazy(() => import('./MyComponent'))", moduleObject);
		...
		*/
		function validateExplicitKey(element, parentType) {
			...
			var childOwner = "";
			if (element && element._owner && element._owner !== ReactCurrentOwner.current) childOwner = " It was passed a child from " + getComponentName(element._owner.type) + ".";
		...
		*/
		function validatePropTypes(element) {
			...
			if (propTypes) {
				...
				error("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", getComponentName(type) || "Unknown");
			}
...
export { require_object_assign as n, require_react as t };

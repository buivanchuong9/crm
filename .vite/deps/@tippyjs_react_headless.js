...
import { n as applyStyles_default, t as createPopper } from "./popper-CpvkFFpD.js";
//#region node_modules/tippy.js/headless/dist/tippy-headless.esm.js
/**!
* tippy.js v6.3.7
* (c) 2017-2021 atomiks
* MIT License
...
function validateProps(partialProps, plugins) {
	...
	Object.keys(partialProps).forEach(function(prop) {
		var didPassUnknownProp = !hasOwnProperty(removeProperties(defaultProps, Object.keys(pluginProps)), prop);
...
var applyStylesModifier = Object.assign({}, applyStyles_default, { effect: function effect(_ref) {
...
var createSingleton = function createSingleton(tippyInstances, optionalProps) {
	...
	function prepareInstance(singleton, target) {
		...
		singleton.setProps(Object.assign({}, overrideProps, { getReferenceClientRect: typeof overrideProps.getReferenceClientRect === "function" ? overrideProps.getReferenceClientRect : function() {
	...
	var singleton = tippy(div(), Object.assign({}, removeProperties(optionalProps, ["overrides"]), {
		...
		popperOptions: Object.assign({}, optionalProps.popperOptions, { modifiers: [].concat(((_optionalProps$popper = optionalProps.popperOptions) == null ? void 0 : _optionalProps$popper.modifiers) || [], [applyStylesModifier]) })
	}));
...
//#endregion
//#region node_modules/@tippyjs/react/headless/dist/tippy-react-headless.esm.js
...
function preserveRef(ref, node) {
	if (ref) {
		...
		if ({}.hasOwnProperty.call(ref, "current")) ref.current = node;
	}
}
...
function toDataAttributes(attrs) {
	var dataAttrs = { "data-placement": attrs.placement };
	if (attrs.referenceHidden) dataAttrs["data-reference-hidden"] = "";
	if (attrs.escaped) dataAttrs["data-escaped"] = "";
	return dataAttrs;
}
...
function deepPreserveProps(instanceProps, componentProps) {
	...
	return Object.assign({}, componentProps, { popperOptions: Object.assign({}, instanceProps.popperOptions, componentProps.popperOptions, { modifiers: uniqueByShape([].concat(((_instanceProps$popper = instanceProps.popperOptions) == null ? void 0 : _instanceProps$popper.modifiers) || [], ((_componentProps$poppe = componentProps.popperOptions) == null ? void 0 : _componentProps$poppe.modifiers) || [])) }) });
}
...
function useMutableBox(initialValue) {
	...
	if (!ref.current) ref.current = typeof initialValue === "function" ? initialValue() : initialValue;
	return ref.current;
}
function updateClassName(box, action, classNames) {
	classNames.split(/\s+/).forEach(function(name) {
		if (name) box.classList[action](name);
	});
}
var classNamePlugin = {
	name: "className",
	...
	fn: function fn(instance) {
		...
		function remove() {
			if (isDefaultRenderFn()) updateClassName(box, "remove", instance.props.className);
		}
		return {
			onCreate: add,
...
function TippyGenerator(tippy) {
	function Tippy(_ref) {
		...
		_ref.__source;
		_ref.__self;
		var restOfNativeProps = _objectWithoutPropertiesLoose(_ref, [
			"children",
			"content",
			"visible",
			"singleton",
		...
		var props = Object.assign({ ignoreAttributes }, restOfNativeProps, { content: mutableBox.container });
		if (isControlledMode) {
			[
				"trigger",
				"hideOnClick",
				...
				if (props[nativeStateProp] !== void 0) console.warn(["@tippyjs/react: Cannot specify `" + nativeStateProp + "` prop in", "controlled mode (`visible` prop)"].join(" "));
			});
			props.trigger = "manual";
			props.hideOnClick = false;
		}
		if (isSingletonMode) disabled = true;
		var computedProps = props;
		var plugins = props.plugins || [];
		if (render) computedProps = Object.assign({}, props, {
			plugins: isSingletonMode && singleton.data != null ? [].concat(plugins, [{ fn: function fn() {
				return { onTrigger: function onTrigger(instance, event) {
					var node = singleton.data.children.find(function(_ref2) {
						return _ref2.instance.reference === event.currentTarget;
					});
			...
			render: function render() {
				return { popper: mutableBox.container };
			}
		});
		var deps = [reference].concat(children ? [children.type] : []);
		useIsomorphicLayoutEffect(function() {
			var element = reference;
			if (reference && reference.hasOwnProperty("current")) element = reference.current;
			var instance = tippy(element || mutableBox.ref || ssrSafeCreateDiv(), Object.assign({}, computedProps, { plugins: [classNamePlugin].concat(props.plugins || []) }));
			mutableBox.instance = instance;
			if (disabled) instance.disable();
			if (visible) instance.show();
			if (isSingletonMode) singleton.hook({
				instance,
				content,
				props: computedProps,
				setSingletonContent
		...
		useIsomorphicLayoutEffect(function() {
			...
			(_instance$popperInsta = instance.popperInstance) == null || _instance$popperInsta.forceUpdate();
			if (disabled) instance.disable();
			else instance.enable();
			if (isControlledMode) if (visible) instance.show();
			else instance.hide();
			if (isSingletonMode) singleton.hook({
				instance,
				content,
		...
		useIsomorphicLayoutEffect(function() {
			...
			if (!render) return;
			var instance = mutableBox.instance;
			instance.setProps({ popperOptions: Object.assign({}, instance.props.popperOptions, { modifiers: [].concat((((_instance$props$poppe = instance.props.popperOptions) == null ? void 0 : _instance$props$poppe.modifiers) || []).filter(function(_ref3) {
				return _ref3.name !== "$$tippyReact";
			}), [{
				name: "$$tippyReact",
				enabled: true,
				phase: "beforeWrite",
				...
				fn: function fn(_ref4) {
					...
					if (attrs.placement !== state.placement || attrs.referenceHidden !== (hideData == null ? void 0 : hideData.isReferenceHidden) || attrs.escaped !== (hideData == null ? void 0 : hideData.hasPopperEscaped)) setAttrs({
		...
		return /* @__PURE__ */ import_react.createElement(import_react.default.Fragment, null, children ? /* @__PURE__ */ (0, import_react.cloneElement)(children, { ref: function ref(node) {
			mutableBox.ref = node;
		...
		useIsomorphicLayoutEffect(function() {
			...
			return function() {
				...
				mutableBox.children = children.filter(function(_ref2) {
					return !_ref2.instance.state.isDestroyed;
				});
			};
		}, [mounted]);
		useIsomorphicLayoutEffect(function() {
			...
			if (!(instance && sourceData)) return;
			var _sourceData$props = sourceData.props;
			_sourceData$props.content;
			var props = _objectWithoutPropertiesLoose(_sourceData$props, ["content"]);
			instance.setProps(deepPreserveProps(instance.props, Object.assign({}, props, { overrides })));
		...
		return (0, import_react.useMemo)(function() {
			return [{
				...
				hook: function hook(data) {
					...
					if (((_mutableBox$instance = mutableBox.instance) == null ? void 0 : _mutableBox$instance.state.isMounted) && ((_mutableBox$instance2 = mutableBox.instance) == null ? void 0 : _mutableBox$instance2.state.$$activeSingletonInstance) === data.instance) mutableBox.setSingletonContent == null || mutableBox.setSingletonContent(data.content);
					if (mutableBox.instance && !mutableBox.instance.state.isDestroyed) mutableBox.instance.setInstances(mutableBox.children.map(function(child) {
						return child.instance;
					}));
				},
...
var forwardRef = (function(Tippy, defaultProps) {
	return /* @__PURE__ */ (0, import_react.forwardRef)(function TippyWrapper(_ref, _ref2) {
		...
		return /* @__PURE__ */ import_react.createElement(Tippy, Object.assign({}, defaultProps, props), children ? /* @__PURE__ */ (0, import_react.cloneElement)(children, { ref: function ref(node) {
...
var headless = /* @__PURE__ */ forwardRef(/* @__PURE__ */ TippyGenerator(tippy), { render: function render() {
	return "";
} });
//#endregion
export { headless as default, tippy, useSingleton };

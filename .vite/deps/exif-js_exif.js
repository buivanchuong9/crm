...
//#region node_modules/exif-js/exif.js
var require_exif = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		...
		if (typeof exports !== "undefined") {
			if (typeof module !== "undefined" && module.exports) exports = module.exports = EXIF;
			exports.EXIF = EXIF;
		} else root.EXIF = EXIF;
		...
		function objectURLToBlob(url, callback) {
			...
			http.onload = function(e) {
				if (this.status == 200 || this.status === 0) callback(this.response);
			};
			http.send();
		}
		function getImageData(img, callback) {
			function handleBinaryFile(binFile) {
				img.exifdata = findEXIFinJPEG(binFile) || {};
				img.iptcdata = findIPTCinJPEG(binFile) || {};
				if (EXIF.isXmpEnabled) img.xmpdata = findXMPinJPEG(binFile) || {};
				if (callback) callback.call(img);
			}
			if (img.src) if (/^data\:/i.test(img.src)) handleBinaryFile(base64ToArrayBuffer(img.src));
		...
		function findIPTCinJPEG(file) {
			...
			var isFieldSegmentStart = function(dataView, offset) {
				return dataView.getUint8(offset) === 56 && dataView.getUint8(offset + 1) === 66 && dataView.getUint8(offset + 2) === 73 && dataView.getUint8(offset + 3) === 77 && dataView.getUint8(offset + 4) === 4 && dataView.getUint8(offset + 5) === 4;
			};
			while (offset < length) {
		...
		function readIPTCData(file, startOffset, sectionLength) {
			var dataView = new DataView(file);
			var data = {}, fieldValue, fieldName, dataSize, segmentType;
			...
			while (segmentStartPos < startOffset + sectionLength) {
				if (dataView.getUint8(segmentStartPos) === 28 && dataView.getUint8(segmentStartPos + 1) === 2) {
					...
					if (segmentType in IptcFieldMap) {
						...
						if (data.hasOwnProperty(fieldName)) if (data[fieldName] instanceof Array) data[fieldName].push(fieldValue);
		...
		function readTagValue(file, entryOffset, tiffStart, dirStart, bigEnd) {
			...
			switch (type) {
				case 1:
				case 7: if (numValues == 1) return file.getUint8(entryOffset + 8, !bigEnd);
				else {
				...
				case 3: if (numValues == 1) return file.getUint16(entryOffset + 8, !bigEnd);
				else {
				...
				case 4: if (numValues == 1) return file.getUint32(entryOffset + 8, !bigEnd);
				else {
					vals = [];
					for (n = 0; n < numValues; n++) vals[n] = file.getUint32(valueOffset + 4 * n, !bigEnd);
					return vals;
				}
				case 5: if (numValues == 1) {
				...
				case 9: if (numValues == 1) return file.getInt32(entryOffset + 8, !bigEnd);
				else {
					vals = [];
					for (n = 0; n < numValues; n++) vals[n] = file.getInt32(valueOffset + 4 * n, !bigEnd);
					return vals;
				}
				case 10: if (numValues == 1) return file.getInt32(valueOffset, !bigEnd) / file.getInt32(valueOffset + 4, !bigEnd);
				else {
					vals = [];
					for (n = 0; n < numValues; n++) vals[n] = file.getInt32(valueOffset + 8 * n, !bigEnd) / file.getInt32(valueOffset + 4 + 8 * n, !bigEnd);
					return vals;
				}
			}
		}
		...
		*/
		...
		function readThumbnailImage(dataView, tiffStart, firstIFDOffset, bigEnd) {
			...
			if (thumbTags["Compression"]) switch (thumbTags["Compression"]) {
				...
				case 1:
					...
					break;
				default: console.log("Unknown thumbnail image format '%s'", thumbTags["Compression"]);
			}
		...
		function readEXIFData(file, start) {
			...
			if (file.getUint16(tiffOffset) == 18761) bigEnd = false;
			else if (file.getUint16(tiffOffset) == 19789) bigEnd = true;
			else {
		...
		function findXMPinJPEG(file) {
			...
			while (offset < length - 4) if (getStringFromDB(dataView, offset, 4) == "http") {
				var xmpString = getStringFromDB(dataView, offset - 1, dataView.getUint16(offset - 2) - 1);
			...
			try {
				...
				if (xml.children.length > 0) for (var i = 0; i < xml.children.length; i++) {
					...
					for (var idx in attributes) {
						...
						if (dataKey !== void 0) obj[dataKey] = dataValue;
					}
					var nodeName = item.nodeName;
					if (typeof obj[nodeName] == "undefined") obj[nodeName] = xml2json(item);
					else {
		...
		EXIF.getAllTags = function(img) {
			...
			for (a in data) if (data.hasOwnProperty(a)) tags[a] = data[a];
			return tags;
		};
		...
		EXIF.pretty = function(img) {
			...
			for (a in data) if (data.hasOwnProperty(a)) if (typeof data[a] == "object") if (data[a] instanceof Number) strPretty += a + " : " + data[a] + " [" + data[a].numerator + "/" + data[a].denominator + "]\r\n";
		...
		if (typeof define === "function" && define.amd) define("exif-js", [], function() {
			return EXIF;
		});
	}).call(exports);
}));
...
//# sourceMappingURL=exif-js_exif.js.map
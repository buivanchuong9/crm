//#region node_modules/is-plain-object/dist/is-plain-object.mjs
/*!
...
var G, U, W = "undefined" != typeof Symbol && "symbol" == typeof Symbol("x"), X = "undefined" != typeof Map, q = "undefined" != typeof Set, B = "undefined" != typeof Proxy && void 0 !== Proxy.revocable && "undefined" != typeof Reflect, H = W ? Symbol.for("immer-nothing") : ((G = {})["immer-nothing"] = !0, G), L = W ? Symbol.for("immer-draftable") : "__$immer_draftable", Q = W ? Symbol.for("immer-state") : "__$immer_state", Y = {
	0: "Illegal state",
...
//#endregion
//#region node_modules/slate/dist/index.es.js
	...
	var editor = {
		...
		apply: (op) => {
			...
			if (Path.operationCanTransformPath(op)) {
				...
				for (var path of oldDirtyPaths) add(Path.transform(path, op));
			} else {
				dirtyPaths = oldDirtyPaths;
				dirtyPathKeys = oldDirtyPathKeys;
			}
			for (var _path of editor.getDirtyPaths(op)) add(_path);
		...
		addMark: (key, value) => {
			...
			if (selection) {
				...
				if (expandedSelection || markAcceptingVoidSelected) Transforms.setNodes(editor, { [key]: value }, {
					match,
					split: true,
					voids: true
				});
				else {
					editor.marks = _objectSpread$9(_objectSpread$9({}, Editor.marks(editor) || {}), {}, { [key]: value });
		...
		deleteForward: (unit) => {
			var { selection } = editor;
			if (selection && Range.isCollapsed(selection)) Transforms.delete(editor, { unit });
		},
		deleteFragment: (direction) => {
			var { selection } = editor;
			if (selection && Range.isExpanded(selection)) Transforms.delete(editor, { reverse: direction === "backward" });
		},
		getFragment: () => {
			var { selection } = editor;
		...
		insertText: (text) => {
			...
			if (selection) {
				if (marks) {
					var node = _objectSpread$9({ text }, marks);
					Transforms.insertNodes(editor, node);
		...
		normalizeNode: (entry) => {
			...
			for (var i = 0; i < node.children.length; i++, n++) {
				...
				if ((Text.isText(_child) || Element.isElement(_child) && editor.isInline(_child)) !== shouldHaveInlines) {
				...
				} else if (prev != null && Text.isText(prev)) {
					if (Text.equals(_child, prev, { loose: true })) {
		...
		*/
		...
		shouldNormalize: (_ref4) => {
			...
			if (iteration > maxIterations) throw new Error("Could not completely normalize the editor after ".concat(maxIterations, " iterations! This is usually due to incorrect normalization logic that leaves a node in an invalid state."));
			return true;
		}
	};
	return editor;
...
var isWordCharacter = function isWordCharacter(char, remaining) {
	...
	if (CHAMELEON.test(char)) {
		var [nextChar, nextRemaining] = splitByCharacterDistance(remaining, getCharacterDistance(remaining, isRTL), isRTL);
...
var getCodepointType = (char, code) => {
	...
	if (code === 8205) type |= CodepointType.ZWJ;
	if (code >= 127462 && code <= 127487) type |= CodepointType.RI;
...
var endsWithOddNumberOfRIs = (str) => {
	...
	else return match[0].length / 2 % 2 === 1;
};
/**
...
var Editor = {
	...
	*/
	above(editor) {
		var { voids = false, mode = "lowest", at = editor.selection, match } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		if (!at) return;
		...
		for (var [n, p] of Editor.levels(editor, {
			...
			if (Range.isRange(at)) {
				if (Path.isAncestor(p, at.anchor.path) && Path.isAncestor(p, at.focus.path)) return [n, p];
	...
	*/
	after(editor, at) {
		...
		var d = 0;
		var target;
		for (var p of Editor.positions(editor, _objectSpread$8(_objectSpread$8({}, options), {}, { at: range }))) {
			if (d > distance) break;
			if (d !== 0) target = p;
			d++;
		}
	...
	*/
	deleteBackward(editor) {
		var { unit = "character" } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
	...
	*/
	deleteFragment(editor) {
		var { direction = "forward" } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
	...
	*/
	isEditor(value) {
		...
		if (cachedIsEditor !== void 0) return cachedIsEditor;
	...
	*levels(editor) {
		...
		for (var [n, p] of Node.levels(editor, path)) {
			...
			levels.push([n, p]);
			if (!voids && Element.isElement(n) && Editor.isVoid(editor, n)) break;
		}
	...
	*/
	marks(editor) {
		...
		if (Range.isExpanded(selection)) {
			...
			if (match) {
				var [_node] = match;
				return _objectWithoutProperties(_node, _excluded$4);
			} else return {};
		}
		...
		if (anchor.offset === 0) {
			...
			if (!Editor.above(editor, { match: (n) => Element.isElement(n) && Editor.isVoid(editor, n) && editor.markableVoid(n) })) {
				var block = Editor.above(editor, { match: (n) => Element.isElement(n) && Editor.isBlock(editor, n) });
				if (prev && block) {
		...
		return _objectWithoutProperties(node, _excluded2$3);
	},
	/**
	...
	*/
	next(editor) {
		...
		if (match == null) if (Path.isPath(at)) {
	...
	*nodes(editor) {
		...
		if (Span.isSpan(at)) {
			...
			var last = Editor.path(editor, at, { edge: "end" });
		...
		for (var [node, path] of nodeEntries) {
			...
			if (!match(node, path)) if (universal && !isLower && Text.isText(node)) return;
			else continue;
	...
	*/
	normalize(editor) {
		var { force = false, operation } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		var getDirtyPaths = (editor) => {
			return DIRTY_PATHS.get(editor) || [];
		};
		var getDirtyPathKeys = (editor) => {
			return DIRTY_PATH_KEYS.get(editor) || /* @__PURE__ */ new Set();
		};
		...
		Editor.withoutNormalizing(editor, () => {
			for (var dirtyPath of getDirtyPaths(editor)) if (Node.has(editor, dirtyPath)) {
				...
				var [node, _] = entry;
				if (Element.isElement(node) && node.children.length === 0) editor.normalizeNode(entry, { operation });
			}
	...
	*/
	path(editor, at) {
		var { depth, edge } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		if (Path.isPath(at)) {
			if (edge === "start") {
		...
		if (Range.isRange(at)) if (edge === "start") at = Range.start(at);
	...
	*/
	pathRef(editor, path) {
		var { affinity = "forward" } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		var ref = {
			current: path,
			affinity,
			unref() {
	...
	*/
	point(editor, at) {
		var { edge = "start" } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		if (Path.isPath(at)) {
			var path;
			if (edge === "end") {
				var [, lastPath] = Node.last(editor, at);
	...
	*/
	*positions(editor) {
		var { at = editor.selection, unit = "offset", reverse = false, voids = false } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		if (!at) return;
		/**
		* Algorithm notes:
		*
	...
	*/
	string(editor, at) {
		var { voids = false } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		var range = Editor.range(editor, at);
		var [start, end] = Range.edges(range);
		var text = "";
		for (var [node, path] of Editor.nodes(editor, {
	...
	*/
	unhangRange(editor, range) {
		var { voids = false } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		var [start, end] = Range.edges(range);
		if (start.offset !== 0 || end.offset !== 0 || Range.isCollapsed(range) || Path.hasPrevious(end.path)) return range;
	...
	*/
	void(editor) {
		var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		return Editor.above(editor, _objectSpread$8(_objectSpread$8({}, options), {}, { match: (n) => Element.isElement(n) && Editor.isVoid(editor, n) }));
	},
	/**
...
var Node = {
	...
	*ancestors(root, path) {
		var options = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		for (var p of Path.ancestors(path, options)) yield [Node.ancestor(root, p), p];
	},
	/**
	...
	*/
	*children(root, path) {
		var { reverse = false } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		var ancestor = Node.ancestor(root, path);
		var { children } = ancestor;
		var index = reverse ? children.length - 1 : 0;
		while (reverse ? index >= 0 : index < children.length) {
			yield [Node.child(ancestor, index), path.concat(index)];
	...
	*descendants(root) {
		var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		for (var [node, path] of Node.nodes(root, options)) if (path.length !== 0) yield [node, path];
	},
	/**
	...
	*elements(root) {
		var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		for (var [node, path] of Node.nodes(root, options)) if (Element.isElement(node)) yield [node, path];
	},
	/**
	...
	*/
	first(root, path) {
		...
		while (n) if (Text.isText(n) || n.children.length === 0) break;
		else {
			n = n.children[0];
			p.push(0);
		}
	...
	*/
	fragment(root, range) {
		...
		return fn({ children: root.children }, (r) => {
			var [start, end] = Range.edges(range);
			for (var [, path] of Node.nodes(r, {
				reverse: true,
				pass: (_ref) => {
					var [, path] = _ref;
	...
	*levels(root, path) {
		var options = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		for (var p of Path.levels(path, options)) yield [Node.get(root, p), p];
	},
	/**
	...
	*texts(root) {
		var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		for (var [node, path] of Node.nodes(root, options)) if (Text.isText(node)) yield [node, path];
	}
};
...
var Operation = {
	...
	*/
	isOperation(value) {
		if (!isPlainObject(value)) return false;
		switch (value.type) {
			case "insert_node": return Path.isPath(value.path) && Node.isNode(value.node);
			case "insert_text": return typeof value.offset === "number" && typeof value.text === "string" && Path.isPath(value.path);
			case "merge_node": return typeof value.position === "number" && Path.isPath(value.path) && isPlainObject(value.properties);
			case "move_node": return Path.isPath(value.path) && Path.isPath(value.newPath);
			case "remove_node": return Path.isPath(value.path) && Node.isNode(value.node);
			case "remove_text": return typeof value.offset === "number" && typeof value.text === "string" && Path.isPath(value.path);
			case "set_node": return Path.isPath(value.path) && isPlainObject(value.properties) && isPlainObject(value.newProperties);
			case "set_selection": return value.properties === null && Range.isRange(value.newProperties) || value.newProperties === null && Range.isRange(value.properties) || isPlainObject(value.properties) && isPlainObject(value.newProperties);
			case "split_node": return Path.isPath(value.path) && typeof value.position === "number" && isPlainObject(value.properties);
	...
	*/
	inverse(op) {
		switch (op.type) {
			case "insert_node": return _objectSpread$7(_objectSpread$7({}, op), {}, { type: "remove_node" });
			case "insert_text": return _objectSpread$7(_objectSpread$7({}, op), {}, { type: "remove_text" });
			case "merge_node": return _objectSpread$7(_objectSpread$7({}, op), {}, {
				type: "split_node",
			...
			case "split_node": return _objectSpread$7(_objectSpread$7({}, op), {}, {
				type: "merge_node",
...
var Path = {
	...
	*/
	isSibling(path, another) {
		...
		return path[path.length - 1] !== another[another.length - 1] && Path.equals(as, bs);
	},
	/**
	...
	*/
	levels(path) {
		var { reverse = false } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		var list = [];
	...
	transform(path, operation) {
		...
		switch (operation.type) {
			...
			case "split_node":
				...
				if (Path.equals(_op3, p)) if (affinity === "forward") p[p.length - 1] += 1;
			...
			case "move_node":
				...
				} else if (Path.isSibling(_op4, onp) && (Path.isAncestor(onp, p) || Path.equals(onp, p))) if (Path.endsBefore(_op4, p)) p[_op4.length - 1] -= 1;
				...
				case "split_node":
					if (Path.equals(op.path, path)) {
						if (op.position === offset && affinity == null) return null;
							...
							p.path = Path.transform(path, op, _objectSpread$6(_objectSpread$6({}, options), {}, { affinity: "forward" }));
						}
	...
	*/
	transform(range, op) {
		...
		return fn(range, (r) => {
			...
			} else if (affinity === "outward") if (Range.isForward(r)) {
				affinityAnchor = "backward";
			...
			var anchor = Point.transform(r.anchor, op, { affinity: affinityAnchor });
			var focus = Point.transform(r.focus, op, { affinity: affinityFocus });
...
var isDeepEqual = (node, another) => {
	...
	for (var _key in another) if (node[_key] === void 0 && another[_key] !== void 0) return false;
	return true;
};
...
var Text = {
	...
	*/
	equals(text, another) {
		var { loose = false } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
		function omitText(obj) {
			return _objectWithoutProperties(obj, _excluded$1);
		}
		...
		for (var dec of decorations) {
			...
			for (var leaf of leaves) {
				...
				if (decorationEnd < leafEnd) {
					...
					after = _objectSpread$4(_objectSpread$4({}, middle), {}, { text: middle.text.slice(off) });
					middle = _objectSpread$4(_objectSpread$4({}, middle), {}, { text: middle.text.slice(0, off) });
				}
				if (decorationStart > leafStart) {
					var _off = decorationStart - leafStart;
					before = _objectSpread$4(_objectSpread$4({}, middle), {}, { text: middle.text.slice(0, _off) });
					middle = _objectSpread$4(_objectSpread$4({}, middle), {}, { text: middle.text.slice(_off) });
				}
...
var applyToDraft = (editor, selection, op) => {
	switch (op.type) {
		...
		case "remove_node":
			...
			if (selection) for (var [_point4, _key4] of Range.points(selection)) {
				...
				if (selection != null && result != null) selection[_key4] = result;
					...
					var _prev = void 0;
					var next = void 0;
					for (var [n, p] of Node.texts(editor)) if (Path.compare(p, _path4) === -1) _prev = [n, p];
					else {
						next = [n, p];
						break;
					}
					var preferNext = false;
					if (_prev && next) if (Path.equals(next[1], _path4)) preferNext = !Path.hasPrevious(next[1]);
		...
		case "remove_text":
			...
			_node4.text = _node4.text.slice(0, _offset) + _node4.text.slice(_offset + _text.length);
		...
		case "split_node":
			...
			if (Text.isText(_node6)) {
				...
				newNode = _objectSpread$3(_objectSpread$3({}, _properties), {}, { text: _after2 });
			} else {
				var _before3 = _node6.children.slice(0, position);
				var _after3 = _node6.children.slice(position);
				_node6.children = _before3;
				newNode = _objectSpread$3(_objectSpread$3({}, _properties), {}, { children: _after3 });
			}
...
transform(editor, op) {
	...
	try {
		...
		if (selection) editor.selection = r(selection) ? dn(selection) : selection;
	...
	*/
	insertNodes(editor, nodes) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (Node.isNode(nodes)) nodes = [nodes];
			...
			if (Range.isRange(at)) {
				if (!hanging) at = Editor.unhangRange(editor, at, { voids });
			...
			if (Point.isPoint(at)) {
				if (match == null) if (Text.isText(node)) match = (n) => Text.isText(n);
				else if (editor.isInline(node)) match = (n) => Text.isText(n) || Editor.isInline(editor, n);
				else match = (n) => Element.isElement(n) && Editor.isBlock(editor, n);
			...
			if (!voids && Editor.void(editor, { at: parentPath })) return;
	...
	*/
	liftNodes(editor) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			var { match } = options;
			if (match == null) match = Path.isPath(at) ? matchPath(editor, at) : (n) => Element.isElement(n) && Editor.isBlock(editor, n);
			if (!at) return;
			...
			for (var pathRef of Array.from(matches, (_ref) => {
				var [, p] = _ref;
	...
	*/
	mergeNodes(editor) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (!hanging && Range.isRange(at)) at = Editor.unhangRange(editor, at, { voids });
			if (Range.isRange(at)) if (Range.isCollapsed(at)) at = at.anchor;
			else {
				var [, end] = Range.edges(at);
				var pointRef = Editor.pointRef(editor, end);
				Transforms.delete(editor, { at });
			...
			var levels = Array.from(Editor.levels(editor, { at: path }), (_ref2) => {
				var [n] = _ref2;
				return n;
	...
	*/
	moveNodes(editor, options) {
		Editor.withoutNormalizing(editor, () => {
			...
			for (var pathRef of Array.from(targets, (_ref3) => {
				var [, p] = _ref3;
				return Editor.pathRef(editor, p);
			})) {
				var path = pathRef.unref();
	...
	*/
	removeNodes(editor) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			for (var pathRef of Array.from(depths, (_ref4) => {
				var [, p] = _ref4;
				return Editor.pathRef(editor, p);
			})) {
				var path = pathRef.unref();
	...
	setNodes(editor, props) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (split && Range.isRange(at)) {
				if (Range.isCollapsed(at) && Editor.leaf(editor, at.anchor)[0].text.length > 0) return;
				var rangeRef = Editor.rangeRef(editor, at, { affinity: "inward" });
				var [start, end] = Range.edges(at);
	...
	*/
	splitNodes(editor) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (match == null) match = (n) => Element.isElement(n) && Editor.isBlock(editor, n);
			...
			if (!at) return;
			var beforeRef = Editor.pointRef(editor, at, { affinity: "backward" });
			var afterRef;
			try {
				...
				for (var [node, _path2] of Editor.levels(editor, {
					...
					var split = false;
					if (_path2.length < highestPath.length || _path2.length === 0 || !voids && Element.isElement(node) && Editor.isVoid(editor, node)) break;
	...
	*/
	wrapNodes(editor, element) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			var { match, at = editor.selection } = options;
			if (!at) return;
			if (match == null) if (Path.isPath(at)) match = matchPath(editor, at);
			else if (editor.isInline(element)) match = (n) => Element.isElement(n) && Editor.isInline(editor, n) || Text.isText(n);
			else match = (n) => Element.isElement(n) && Editor.isBlock(editor, n);
			if (split && Range.isRange(at)) {
				var [start, end] = Range.edges(at);
				var rangeRef = Editor.rangeRef(editor, at, { affinity: "inward" });
			...
			for (var [, rootPath] of Array.from(Editor.nodes(editor, {
				at,
				...
				if (matches.length > 0) {
					if (function() {
						...
						var [, firstPath] = first;
						var [, lastPath] = last;
						if (firstPath.length === 0 && lastPath.length === 0) return "continue";
						var commonPath = Path.equals(firstPath, lastPath) ? Path.parent(firstPath) : Path.common(firstPath, lastPath);
						var range = Editor.range(editor, firstPath, lastPath);
						var [commonNode] = Editor.node(editor, commonPath);
						var depth = commonPath.length + 1;
						var wrapperPath = Path.next(lastPath.slice(0, depth));
						var wrapper = _objectSpread$2(_objectSpread$2({}, element), {}, { children: [] });
...
var SelectionTransforms = {
	...
	*/
	collapse(editor) {
		var { edge = "anchor" } = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
		var { selection } = editor;
		if (!selection) return;
	...
	*/
	setPoint(editor, props) {
		...
		Transforms.setSelection(editor, { [edge === "anchor" ? "anchor" : "focus"]: _objectSpread$1(_objectSpread$1({}, point), props) });
	},
	/**
	...
	delete(editor) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (!hanging) {
				...
				if (!Point.equals(_end, endOfDoc)) at = Editor.unhangRange(editor, at, { voids });
			}
			var [start, end] = Range.edges(at);
	...
	*/
	insertFragment(editor, fragment) {
		...
		Editor.withoutNormalizing(editor, () => {
			...
			if (!voids && Editor.void(editor, { at })) return;
			...
			if (inlineElementMatch) {
				...
				if (Editor.isEnd(editor, at, _inlinePath)) at = Editor.after(editor, _inlinePath);
				else if (Editor.isStart(editor, at, _inlinePath)) at = Editor.before(editor, _inlinePath);
			}
			var [, blockPath] = Editor.above(editor, {
				match: (n) => Element.isElement(n) && Editor.isBlock(editor, n),
				at,
				voids
			});
			...
			var [, firstPath] = Node.first({ children: fragment }, []);
			var [, lastPath] = Node.last({ children: fragment }, []);
			var matches = [];
			var matcher = (_ref2) => {
				var [n, p] = _ref2;
			...
			for (var entry of Node.nodes({ children: fragment }, { pass: matcher })) if (matcher(entry)) matches.push(entry);
			var starts = [];
			var middles = [];
			var ends = [];
			var starting = true;
			var hasBlocks = false;
			for (var [node] of matches) if (Element.isElement(node) && !editor.isInline(node)) {
				starting = false;
				hasBlocks = true;
				middles.push(node);
...
//#endregion
export { isPlainObject as _, Operation as a, Point as c, RangeRef as d, Scrubber as f, createEditor as g, Transforms as h, Node as i, PointRef as l, Text as m, Element as n, Path as o, Span as p, Location as r, PathRef as s, Editor as t, Range as u };

//# sourceMappingURL=index.es-Cvt3Mw6e.js.map
...
var import_direction = /* @__PURE__ */ __toESM(require_direction());
...
var i = function(e, i) {
	...
	for (var a, c, g = document.scrollingElement || document.documentElement, p = [], m = e; t(m) && s(m);) {
		...
		null != m && m === document.body && n$1(m) && !n$1(document.documentElement) || null != m && n$1(m, u) && p.push(m);
	}
...
function scrollIntoView(target, options) {
	...
	if (isOptionsObject(options) && typeof options.behavior === "function") return options.behavior(isTargetAttached ? i(target, options) : []);
...
//#endregion
//#region node_modules/slate-react/dist/index.es.js
	...
	function parseHotkey(hotkey, options) {
		...
		try {
			for (var _iterator = values[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
				...
				if (length === 1 || !modifier) if (byKey) ret.key = name;
...
var getSlateFragmentAttribute = (dataTransfer) => {
	var [, fragment] = dataTransfer.getData("text/html").match(catchSlateFragment) || [];
	return fragment;
};
/**
...
var isTrackedMutation = (editor, mutation, batch) => {
	...
	var { document } = ReactEditor.getWindow(editor);
	if (document.contains(target)) return ReactEditor.hasDOMNode(editor, target, { editable: true });
	var parentMutation = batch.find((_ref) => {
		var { addedNodes, removedNodes } = _ref;
		for (var node of addedNodes) if (node === target || node.contains(target)) return true;
		for (var _node of removedNodes) if (_node === target || _node.contains(target)) return true;
	});
	if (!parentMutation || parentMutation === mutation) return false;
	return isTrackedMutation(editor, parentMutation, batch);
};
var IS_REACT_VERSION_17_OR_ABOVE = parseInt(import_react.version.split(".")[0], 10) >= 17;
...
var ReactEditor = {
	...
	*/
	getWindow(editor) {
		...
		if (!window) throw new Error("Unable to find a host window element for this editor");
		return window;
	},
	/**
	...
	*/
	findPath(editor, node) {
		...
		while (true) {
			...
			if (parent == null) if (Editor.isEditor(child)) return path;
			else break;
	...
	*/
	deselect(editor) {
		...
		var domSelection = ReactEditor.findDocumentOrShadowRoot(editor).getSelection();
		if (domSelection && domSelection.rangeCount > 0) domSelection.removeAllRanges();
		if (selection) Transforms.deselect(editor);
	},
	/**
	* Check if a DOM node is within the editor.
	*/
	hasDOMNode(editor, target) {
		var { editable = false } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
	...
	*/
	toDOMNode(editor, node) {
		...
		if (!domNode) throw new Error("Cannot resolve a DOM node from Slate node: ".concat(Scrubber.stringify(node)));
		return domNode;
	},
	/**
	...
	*/
	toDOMPoint(editor, point) {
		...
		var domPoint;
		if (Editor.void(editor, { at: point })) point = {
			path: point.path,
			offset: 0
		};
		var texts = Array.from(el.querySelectorAll("[data-slate-string], [data-slate-zero-width]"));
		var start = 0;
		for (var i = 0; i < texts.length; i++) {
			...
			var end = start + (attr == null ? length : parseInt(attr, 10));
		...
		if (!domPoint) throw new Error("Cannot resolve a DOM point from Slate point: ".concat(Scrubber.stringify(point)));
		return domPoint;
	},
	/**
	...
	*/
	toDOMRange(editor, range) {
		...
		var domRange = ReactEditor.getWindow(editor).document.createRange();
		var [startNode, startOffset] = isBackward ? domFocus : domAnchor;
		var [endNode, endOffset] = isBackward ? domAnchor : domFocus;
		var isStartAtZeroWidth = !!(isDOMElement(startNode) ? startNode : startNode.parentElement).getAttribute("data-slate-zero-width");
		var isEndAtZeroWidth = !!(isDOMElement(endNode) ? endNode : endNode.parentElement).getAttribute("data-slate-zero-width");
	...
	*/
	findEventRange(editor, event) {
		...
		if (Element$2.isElement(node) && Editor.isVoid(editor, node)) {
			...
			var edge = Editor.point(editor, path, { edge: isPrev ? "start" : "end" });
			var point = isPrev ? Editor.before(editor, edge) : Editor.after(editor, edge);
			if (point) return Editor.range(editor, point);
		}
		var domRange;
		var { document } = ReactEditor.getWindow(editor);
		if (document.caretRangeFromPoint) domRange = document.caretRangeFromPoint(x, y);
		else {
	...
	toSlatePoint(editor, domPoint, options) {
		...
		if (parentNode) {
			...
			if (leafNode) {
				...
				if (textNode) {
					var range = ReactEditor.getWindow(editor).document.createRange();
		...
		if (IS_ANDROID && !textNode && !exactMatch) {
			...
			if (node && ReactEditor.hasDOMNode(editor, node, { editable: true })) {
				...
				var { path: _path, offset: _offset } = Editor.start(editor, ReactEditor.findPath(editor, _slateNode));
	...
	*/
	toSlateRange(editor, domRange, options) {
		...
		var focusOffset;
		var isCollapsed;
		if (el) if (isDOMSelection(domRange)) {
			anchorNode = domRange.anchorNode;
			anchorOffset = domRange.anchorOffset;
			focusNode = domRange.focusNode;
			focusOffset = domRange.focusOffset;
			if (IS_CHROME && hasShadowRoot(anchorNode)) isCollapsed = domRange.anchorNode === domRange.focusNode && domRange.anchorOffset === domRange.focusOffset;
		...
		*/
		if (IS_FIREFOX && !isCollapsed && anchorNode !== focusNode) {
			...
			if (isEnd) anchor = Editor.after(editor, anchor) || anchor;
			if (isStart) focus = Editor.before(editor, focus) || focus;
		}
		var range = {
			anchor,
			focus
...
var isElementDecorationsEqual = (list, another) => {
	...
	for (var i = 0; i < list.length; i++) {
		var range = list[i];
		var other = another[i];
		if (!Range.equals(range, other) || !isDecorationFlagsEqual(range, other)) return false;
	}
	return true;
};
/**
...
var isTextDecorationsEqual = (list, another) => {
	...
	for (var i = 0; i < list.length; i++) {
		var range = list[i];
		var other = another[i];
		if (range.anchor.offset !== other.anchor.offset || range.focus.offset !== other.focus.offset || !isDecorationFlagsEqual(range, other)) return false;
	}
	return true;
};
/**
...
var String$1 = (props) => {
	...
	if (editor.isVoid(parent)) return /* @__PURE__ */ import_react.createElement(ZeroWidthString, { length: Node.string(parent).length });
	if (leaf.text === "" && parent.children[parent.children.length - 1] === text && !editor.isInline(parent) && Editor.string(editor, parentPath) === "") return /* @__PURE__ */ import_react.createElement(ZeroWidthString, {
		isLineBreak: true,
		isMarkPlaceholder
	});
	if (leaf.text === "") return /* @__PURE__ */ import_react.createElement(ZeroWidthString, { isMarkPlaceholder });
	if (isLast && leaf.text.slice(-1) === "\n") return /* @__PURE__ */ import_react.createElement(TextString, {
		isTrailing: true,
		text: leaf.text
	});
	return /* @__PURE__ */ import_react.createElement(TextString, { text: leaf.text });
};
/**
...
var TextString = (props) => {
	...
	return /* @__PURE__ */ import_react.createElement(MemoizedText$1, { ref }, initialText);
};
...
var ZeroWidthString = (props) => {
	...
	return /* @__PURE__ */ import_react.createElement("span", Object.assign({}, attributes), !IS_ANDROID || !isLineBreak ? "﻿" : null, isLineBreak ? /* @__PURE__ */ import_react.createElement("br", null) : null);
};
/**
...
*/
var Leaf = (props) => {
	var { leaf, isLast, text, parent, renderPlaceholder, renderLeaf = (props) => /* @__PURE__ */ import_react.createElement(DefaultLeaf, Object.assign({}, props)) } = props;
	...
	(0, import_react.useEffect)(() => {
		return () => {
			if (placeholderResizeObserver.current) placeholderResizeObserver.current.disconnect();
		};
	}, []);
	(0, import_react.useEffect)(() => {
		var placeholderEl = placeholderRef === null || placeholderRef === void 0 ? void 0 : placeholderRef.current;
		if (placeholderEl) EDITOR_TO_PLACEHOLDER_ELEMENT.set(editor, placeholderEl);
		else EDITOR_TO_PLACEHOLDER_ELEMENT.delete(editor);
		if (placeholderResizeObserver.current) {
			placeholderResizeObserver.current.disconnect();
			if (placeholderEl) placeholderResizeObserver.current.observe(placeholderEl);
		} else if (placeholderEl) {
			placeholderResizeObserver.current = new (window.ResizeObserver || ResizeObserver)(() => {
	...
	var children = /* @__PURE__ */ import_react.createElement(String$1, {
		isLast,
		leaf,
		parent,
		text
	...
	if (leaf[PLACEHOLDER_SYMBOL]) {
		...
		children = /* @__PURE__ */ import_react.createElement(import_react.default.Fragment, null, renderPlaceholder(placeholderProps), children);
	}
	return renderLeaf({
...
var MemoizedLeaf = /* @__PURE__ */ import_react.memo(Leaf, (prev, next) => {
...
var DefaultLeaf = (props) => {
	...
	return /* @__PURE__ */ import_react.createElement("span", Object.assign({}, attributes), children);
};
/**
* Text.
*/
var Text = (props) => {
	...
	for (var i = 0; i < leaves.length; i++) {
		var leaf = leaves[i];
		children.push(/* @__PURE__ */ import_react.createElement(MemoizedLeaf, {
	...
	var callbackRef = (0, import_react.useCallback)((span) => {
		...
		if (span) {
			...
			if (ref.current) ELEMENT_TO_NODE.delete(ref.current);
		}
		ref.current = span;
	}, [
		ref,
...
var MemoizedText = /* @__PURE__ */ import_react.memo(Text, (prev, next) => {
...
*/
var Element$1 = (props) => {
	var { decorations, element, renderElement = (p) => /* @__PURE__ */ import_react.createElement(DefaultElement, Object.assign({}, p)), renderPlaceholder, renderLeaf, selection } = props;
	var editor = useSlateStatic();
	var readOnly = useReadOnly();
	var isInline = editor.isInline(element);
	var key = ReactEditor.findKey(editor, element);
	var ref = (0, import_react.useCallback)((ref) => {
		var KEY_TO_ELEMENT = EDITOR_TO_KEY_TO_ELEMENT.get(editor);
		if (ref) {
	...
	if (!isInline && Editor.hasInlines(editor, element)) {
		var dir = (0, import_direction.default)(Node.string(element));
	...
	if (Editor.isVoid(editor, element)) {
		...
		children = /* @__PURE__ */ import_react.createElement(Tag, {
		...
		}, /* @__PURE__ */ import_react.createElement(MemoizedText, {
			renderPlaceholder,
			decorations: [],
			isLast: false,
			parent: element,
...
var MemoizedElement = /* @__PURE__ */ import_react.memo(Element$1, (prev, next) => {
...
var DefaultElement = (props) => {
	...
	var Tag = useSlateStatic().isInline(element) ? "span" : "div";
	return /* @__PURE__ */ import_react.createElement(Tag, Object.assign({}, attributes, { style: { position: "relative" } }), children);
};
/**
...
var useChildren = (props) => {
	...
	for (var i = 0; i < node.children.length; i++) {
		...
		if (Element$2.isElement(n)) children.push(/* @__PURE__ */ import_react.createElement(SelectedContext.Provider, {
			key: "provider-".concat(key.id),
			value: !!sel
		}, /* @__PURE__ */ import_react.createElement(MemoizedElement, {
			decorations: ds,
			element: n,
			key: key.id,
			renderElement,
		...
		else children.push(/* @__PURE__ */ import_react.createElement(MemoizedText, {
			decorations: ds,
			key: key.id,
...
var create = (key) => {
	...
	var isGeneric = generic && (0, import_lib.isKeyHotkey)(generic);
	var isApple = apple && (0, import_lib.isKeyHotkey)(apple);
	var isWindows = windows && (0, import_lib.isKeyHotkey)(windows);
	return (event) => {
...
var RestoreDOM = IS_ANDROID ? RestoreDOMComponent : (_ref) => {
	var { children } = _ref;
	return /* @__PURE__ */ import_react.createElement(import_react.default.Fragment, null, children);
};
/**
...
function verifyDiffState(editor, textDiff) {
	...
	if (!Text$1.isText(node)) return false;
...
function longestCommonPrefixLength(str, another) {
	...
	for (var i = 0; i < length; i++) if (str.charAt(i) !== another.charAt(i)) return i;
	return length;
}
function longestCommonSuffixLength(str, another, max) {
	var length = Math.min(str.length, another.length, max);
	for (var i = 0; i < length; i++) if (str.charAt(str.length - i - 1) !== another.charAt(another.length - i - 1)) return i;
	return length;
}
/**
...
function normalizeStringDiff(targetText, diff) {
	...
	var suffixLength = longestCommonSuffixLength(removedText, text, Math.min(removedText.length - prefixLength, text.length - prefixLength));
	var normalized = {
...
function normalizePoint(editor, point) {
	...
	if (!Text$1.isText(leaf)) return null;
	...
	while (offset > leaf.text.length) {
		...
		if (!entry || !Path.isDescendant(entry[1], parentBlock[1])) return null;
...
function transformPendingPoint(editor, point, op) {
	...
	if (!textDiff || point.offset <= textDiff.diff.start) return Point.transform(point, op, { affinity: "backward" });
	var { diff } = textDiff;
	if (point.offset <= diff.start + diff.text.length) {
		...
		var _transformed = Point.transform(_anchor, op, { affinity: "backward" });
	...
	var transformed = Point.transform(anchor, op, { affinity: "backward" });
	if (!transformed) return null;
	if (op.type === "split_node" && Path.equals(op.path, point.path) && anchor.offset < op.position && diff.start < op.position) return transformed;
	return {
...
function transformTextDiff(textDiff, op) {
	...
	switch (op.type) {
		case "insert_text":
			if (!Path.equals(op.path, path) || op.offset >= diff.end) return textDiff;
		...
		case "split_node":
			if (!Path.equals(op.path, path) || op.position >= diff.end) return {
				diff,
				id,
		...
		case "merge_node":
			if (!Path.equals(op.path, path)) return {
				diff,
				id,
...
function createAndroidInputManager(_ref) {
	...
	var applyPendingSelection = () => {
		...
		if (pendingSelection) {
			...
			if (normalized && (!selection || !Range.equals(normalized, selection))) Transforms.select(editor, normalized);
		}
	};
	var performAction = () => {
		...
		if (action.at) {
			...
			if (!editor.selection || !Range.equals(editor.selection, _targetRange)) Transforms.select(editor, target);
		}
		action.run();
	};
	var flush = () => {
		...
		while (diff = (_EDITOR_TO_PENDING_DI = EDITOR_TO_PENDING_DIFFS.get(editor)) === null || _EDITOR_TO_PENDING_DI === void 0 ? void 0 : _EDITOR_TO_PENDING_DI[0]) {
			...
			if (!editor.selection || !Range.equals(editor.selection, range)) Transforms.select(editor, range);
			if (diff.diff.text) Editor.insertText(editor, diff.diff.text);
		...
		if (selection && !EDITOR_TO_PENDING_SELECTION.get(editor) && (!editor.selection || !Range.equals(selection, editor.selection))) Transforms.select(editor, selection);
		if (hasPendingAction()) {
			performAction();
			return;
		}
	...
	var storeDiff = (path, diff) => {
		...
		if (idx < 0) {
			if (normalizeStringDiff(target.text, diff)) pendingDiffs.push({
				path,
				diff,
				id: idCounter++
			});
		...
		pendingDiffs[idx] = _objectSpread$3(_objectSpread$3({}, pendingDiffs[idx]), {}, { diff: merged });
	};
	...
	var handleDOMBeforeInput = (event) => {
		...
		if (type.startsWith("delete")) {
			if (Range.isExpanded(targetRange)) {
				...
				if (Node.leaf(editor, _start.path).text.length === _start.offset && _end.offset === 0) {
					var next = Editor.next(editor, {
						at: _start.path,
						match: Text$1.isText
					});
					if (next && Path.equals(next[1], _end.path)) targetRange = {
						anchor: _end,
						focus: _end
					};
				}
			...
			if (applyStringDiff(leaf.text, ...diffs).length === 0) canStoreDiff = false;
			if (Range.isExpanded(targetRange)) {
				if (canStoreDiff && Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
					...
					handleUserSelect(Editor.range(editor, point, point));
				...
				return scheduleAction(() => Editor.deleteFragment(editor, { direction }), { at: targetRange });
			}
		}
		switch (type) {
			case "deleteByComposition":
			case "deleteByCut":
			case "deleteByDrag": return scheduleAction(() => Editor.deleteFragment(editor), { at: targetRange });
			case "deleteContent":
			case "deleteContentForward":
				...
				return scheduleAction(() => Editor.deleteForward(editor), { at: targetRange });
			case "deleteContentBackward":
				var _nativeTargetRange;
				var { anchor: _anchor } = targetRange;
				var nativeCollapsed = isDOMSelection(nativeTargetRange) ? nativeTargetRange.isCollapsed : !!((_nativeTargetRange = nativeTargetRange) !== null && _nativeTargetRange !== void 0 && _nativeTargetRange.collapsed);
				if (canStoreDiff && nativeCollapsed && Range.isCollapsed(targetRange) && _anchor.offset > 0) return storeDiff(_anchor.path, {
					text: "",
					start: _anchor.offset - 1,
					end: _anchor.offset
				});
				return scheduleAction(() => Editor.deleteBackward(editor), { at: targetRange });
			case "deleteEntireSoftLine": return scheduleAction(() => {
				Editor.deleteBackward(editor, { unit: "line" });
				Editor.deleteForward(editor, { unit: "line" });
			}, { at: targetRange });
			case "deleteHardLineBackward": return scheduleAction(() => Editor.deleteBackward(editor, { unit: "block" }), { at: targetRange });
			case "deleteSoftLineBackward": return scheduleAction(() => Editor.deleteBackward(editor, { unit: "line" }), { at: targetRange });
			case "deleteHardLineForward": return scheduleAction(() => Editor.deleteForward(editor, { unit: "block" }), { at: targetRange });
			case "deleteSoftLineForward": return scheduleAction(() => Editor.deleteForward(editor, { unit: "line" }), { at: targetRange });
			case "deleteWordBackward": return scheduleAction(() => Editor.deleteBackward(editor, { unit: "word" }), { at: targetRange });
			case "deleteWordForward": return scheduleAction(() => Editor.deleteForward(editor, { unit: "word" }), { at: targetRange });
			case "insertLineBreak": return scheduleAction(() => Editor.insertSoftBreak(editor), { at: targetRange });
			case "insertParagraph": return scheduleAction(() => Editor.insertBreak(editor), { at: targetRange });
			...
			case "insertText":
				if (isDataTransfer(data)) return scheduleAction(() => ReactEditor.insertData(editor, data), { at: targetRange });
				var _text = data !== null && data !== void 0 ? data : "";
				if (EDITOR_TO_PENDING_INSERTION_MARKS.get(editor)) _text = _text.replace("﻿", "");
				if (type === "insertText" && /.*\n.*\n$/.test(_text)) _text = _text.slice(0, -1);
				if (_text.includes("\n")) return scheduleAction(() => {
					var parts = _text.split("\n");
					parts.forEach((line, i) => {
						if (line) Editor.insertText(editor, line);
						if (i !== parts.length - 1) Editor.insertSoftBreak(editor);
					});
				}, { at: targetRange });
				if (Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
					...
					if (_text && insertPositionHint && type === "insertCompositionText") {
						...
						if (_diff.start + _diff.text.search(/\S|$/) === hintPosition + 1 && _diff.end === insertPositionHint.start + insertPositionHint.text.length) {
							_diff.start -= 1;
							insertPositionHint = null;
							scheduleFlush();
						} else insertPositionHint = false;
					} else if (type === "insertText") if (insertPositionHint === null) insertPositionHint = _diff;
					else if (insertPositionHint && Range.isCollapsed(targetRange) && insertPositionHint.end + insertPositionHint.text.length === _start2.offset) insertPositionHint = _objectSpread$3(_objectSpread$3({}, insertPositionHint), {}, { text: insertPositionHint.text + _text });
					else insertPositionHint = false;
					else insertPositionHint = false;
					if (canStoreDiff) {
				...
				return scheduleAction(() => Editor.insertText(editor, _text), { at: targetRange });
		}
	};
...
function useAndroidInputManager(_ref) {
	...
	var [inputManager] = (0, import_react.useState)(() => createAndroidInputManager(_objectSpread$2({ editor }, options)));
...
var Children = (props) => /* @__PURE__ */ import_react.createElement(import_react.default.Fragment, null, useChildren(props));
/**
* Editable.
*/
var Editable = (props) => {
	var _EDITOR_TO_PLACEHOLDE, _EDITOR_TO_PLACEHOLDE2;
	var defaultRenderPlaceholder = (0, import_react.useCallback)((props) => /* @__PURE__ */ import_react.createElement(DefaultPlaceholder, Object.assign({}, props)), []);
	...
	var onDOMSelectionChange = (0, import_react.useCallback)((0, import_throttle.default)(() => {
		if ((IS_ANDROID || !ReactEditor.isComposing(editor)) && (!state.isUpdatingSelection || androidInputManager !== null && androidInputManager !== void 0 && androidInputManager.isFlushing()) && !state.isDraggingInternally) {
			...
			if (anchorNodeSelectable && focusNodeSelectable) {
				...
				if (range) if (!ReactEditor.isComposing(editor) && !(androidInputManager !== null && androidInputManager !== void 0 && androidInputManager.hasPendingChanges()) && !(androidInputManager !== null && androidInputManager !== void 0 && androidInputManager.isFlushing())) Transforms.select(editor, range);
				else androidInputManager === null || androidInputManager === void 0 || androidInputManager.handleUserSelect(range);
			}
			if (readOnly && (!anchorNodeSelectable || !focusNodeSelectable)) Transforms.deselect(editor);
		}
	}, 100), [readOnly]);
	...
	useIsomorphicLayoutEffect(() => {
		...
		var setDomSelection = (forceChange) => {
			...
			if (newDomRange) {
				if (Range.isBackward(selection)) domSelection.setBaseAndExtent(newDomRange.endContainer, newDomRange.endOffset, newDomRange.startContainer, newDomRange.startOffset);
		...
		if (!IS_ANDROID || !ensureSelection) {
			setTimeout(() => {
				if (newDomRange && IS_FIREFOX) ReactEditor.toDOMNode(editor, editor).focus();
		...
		if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isDOMEventHandled(event, propsOnDOMBeforeInput)) {
			...
			if (type === "insertText" && selection && Range.isCollapsed(selection) && event.data && event.data.length === 1 && /[a-z ]/i.test(event.data) && selection.anchor.offset !== 0) {
				...
				if (native && node.parentElement && (window === null || window === void 0 ? void 0 : (_window$getComputedSt = window.getComputedStyle(node.parentElement)) === null || _window$getComputedSt === void 0 ? void 0 : _window$getComputedSt.whiteSpace) === "pre") {
					...
					if (block && Node.string(block[0]).includes("	")) native = false;
				}
			}
			...
			switch (type) {
				...
				case "insertText":
					...
					else if (typeof data === "string") if (native) deferredOperations.current.push(() => Editor.insertText(editor, data));
					else Editor.insertText(editor, data);
					break;
			}
			var toRestore = (_EDITOR_TO_USER_SELEC = EDITOR_TO_USER_SELECTION.get(editor)) === null || _EDITOR_TO_USER_SELEC === void 0 ? void 0 : _EDITOR_TO_USER_SELEC.unref();
			EDITOR_TO_USER_SELECTION.delete(editor);
			if (toRestore && (!editor.selection || !Range.equals(editor.selection, toRestore))) Transforms.select(editor, toRestore);
		}
	...
	var callbackRef = (0, import_react.useCallback)((node) => {
		if (node == null) {
			...
			if (ref.current && HAS_BEFORE_INPUT_SUPPORT) ref.current.removeEventListener("beforeinput", onDOMBeforeInput);
		} else if (HAS_BEFORE_INPUT_SUPPORT) node.addEventListener("beforeinput", onDOMBeforeInput);
		ref.current = node;
	}, [
		ref,
		onDOMBeforeInput,
	...
	if (editor.selection && Range.isCollapsed(editor.selection) && marks) {
		...
		if (!Text$1.equals(leaf, marks, { loose: true })) {
			state.hasMarkPlaceholder = true;
			var unset = Object.fromEntries(Object.keys(rest).map((mark) => [mark, null]));
			decorations.push(_objectSpread$1(_objectSpread$1(_objectSpread$1({ [MARK_PLACEHOLDER_SYMBOL]: true }, unset), marks), {}, {
				anchor,
				focus: anchor
			}));
		}
	...
	(0, import_react.useEffect)(() => {
		setTimeout(() => {
			...
			if (selection) {
				...
				if (marks && !Text$1.equals(_text, marks, { loose: true })) {
	...
	return /* @__PURE__ */ import_react.createElement(ReadOnlyContext.Provider, { value: readOnly }, /* @__PURE__ */ import_react.createElement(DecorateContext.Provider, { value: decorate }, /* @__PURE__ */ import_react.createElement(RestoreDOM, {
		node: ref,
		receivedUserInput
	}, /* @__PURE__ */ import_react.createElement(Component, Object.assign({
		...
		style: _objectSpread$1(_objectSpread$1({}, disableDefaultStyles ? {} : _objectSpread$1({
			...
			whiteSpace: "pre-wrap",
			wordWrap: "break-word"
		}, placeholderHeight ? { minHeight: placeholderHeight } : {})), userStyle),
		...
		onInput: (0, import_react.useCallback)((event) => {
			if (isEventHandled(event, attributes.onInput)) return;
		...
		onBlur: (0, import_react.useCallback)((event) => {
			if (readOnly || state.isUpdatingSelection || !ReactEditor.hasSelectableTarget(editor, event.target) || isEventHandled(event, attributes.onBlur)) return;
			var root = ReactEditor.findDocumentOrShadowRoot(editor);
			if (state.latestElement === root.activeElement) return;
			var { relatedTarget } = event;
			if (relatedTarget === ReactEditor.toDOMNode(editor, editor)) return;
			if (isDOMElement(relatedTarget) && relatedTarget.hasAttribute("data-slate-spacer")) return;
			if (relatedTarget != null && isDOMNode(relatedTarget) && ReactEditor.hasDOMNode(editor, relatedTarget)) {
				var node = ReactEditor.toSlateNode(editor, relatedTarget);
				if (Element$2.isElement(node) && !editor.isVoid(node)) return;
			}
			if (IS_SAFARI) {
				var domSelection = root.getSelection();
		...
		onClick: (0, import_react.useCallback)((event) => {
			if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && isDOMNode(event.target)) {
				var node = ReactEditor.toSlateNode(editor, event.target);
				var path = ReactEditor.findPath(editor, node);
				if (!Editor.hasPath(editor, path) || Node.get(editor, path) !== node) return;
				...
				var startVoid = Editor.void(editor, { at: _start });
				var endVoid = Editor.void(editor, { at: end });
		...
		onCompositionEnd: (0, import_react.useCallback)((event) => {
			if (ReactEditor.hasSelectableTarget(editor, event.target)) {
				...
				if (!IS_SAFARI && !IS_FIREFOX_LEGACY && !IS_IOS && !IS_WECHATBROWSER && !IS_UC_MOBILE && event.data) {
					...
					var userMarks = EDITOR_TO_USER_MARKS.get(editor);
					EDITOR_TO_USER_MARKS.delete(editor);
					if (userMarks !== void 0) editor.marks = userMarks;
				}
			}
		...
		onCut: (0, import_react.useCallback)((event) => {
			if (!readOnly && ReactEditor.hasSelectableTarget(editor, event.target) && !isEventHandled(event, attributes.onCut)) {
				...
				var { selection } = editor;
				if (selection) if (Range.isExpanded(selection)) Editor.deleteFragment(editor);
				else {
					var node = Node.parent(editor, selection.anchor.path);
					if (Editor.isVoid(editor, node)) Transforms.delete(editor);
				}
			}
		...
		onDragOver: (0, import_react.useCallback)((event) => {
			if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDragOver)) {
				var node = ReactEditor.toSlateNode(editor, event.target);
				if (Element$2.isElement(node) && Editor.isVoid(editor, node)) event.preventDefault();
			}
		...
		onDragStart: (0, import_react.useCallback)((event) => {
			if (!readOnly && ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDragStart)) {
				var node = ReactEditor.toSlateNode(editor, event.target);
				var path = ReactEditor.findPath(editor, node);
				if (Element$2.isElement(node) && Editor.isVoid(editor, node) || Editor.void(editor, {
					at: path,
					voids: true
				})) {
		...
		onDragEnd: (0, import_react.useCallback)((event) => {
			if (!readOnly && state.isDraggingInternally && attributes.onDragEnd && ReactEditor.hasTarget(editor, event.target)) attributes.onDragEnd(event);
			state.isDraggingInternally = false;
		}, [readOnly, attributes.onDragEnd]),
		onFocus: (0, import_react.useCallback)((event) => {
			if (!readOnly && !state.isUpdatingSelection && ReactEditor.hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onFocus)) {
				var el = ReactEditor.toDOMNode(editor, editor);
				state.latestElement = ReactEditor.findDocumentOrShadowRoot(editor).activeElement;
		...
		onKeyDown: (0, import_react.useCallback)((event) => {
			if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target)) {
				...
				if (Hotkeys.isMoveBackward(nativeEvent)) {
					event.preventDefault();
					if (selection && Range.isCollapsed(selection)) Transforms.move(editor, { reverse: !isRTL });
				...
				if (Hotkeys.isMoveWordBackward(nativeEvent)) {
					event.preventDefault();
					if (selection && Range.isExpanded(selection)) Transforms.collapse(editor, { edge: "focus" });
					Transforms.move(editor, {
						unit: "word",
						reverse: !isRTL
					});
				...
				if (!HAS_BEFORE_INPUT_SUPPORT) {
					...
					if (Hotkeys.isDeleteBackward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "backward" });
						else Editor.deleteBackward(editor);
						return;
					}
					if (Hotkeys.isDeleteForward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "forward" });
						else Editor.deleteForward(editor);
						return;
					}
					if (Hotkeys.isDeleteLineBackward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "backward" });
						else Editor.deleteBackward(editor, { unit: "line" });
						return;
					}
					if (Hotkeys.isDeleteLineForward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "forward" });
						else Editor.deleteForward(editor, { unit: "line" });
						return;
					}
					if (Hotkeys.isDeleteWordBackward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "backward" });
						else Editor.deleteBackward(editor, { unit: "word" });
						return;
					}
					if (Hotkeys.isDeleteWordForward(nativeEvent)) {
						event.preventDefault();
						if (selection && Range.isExpanded(selection)) Editor.deleteFragment(editor, { direction: "forward" });
						else Editor.deleteForward(editor, { unit: "word" });
						return;
					}
	...
	}), /* @__PURE__ */ import_react.createElement(Children, {
		decorations,
		node: editor,
		renderElement,
		renderPlaceholder,
...
var DefaultPlaceholder = (_ref) => {
	...
	return /* @__PURE__ */ import_react.createElement("span", Object.assign({}, attributes), children, IS_ANDROID && /* @__PURE__ */ import_react.createElement("br", null));
};
/**
...
function getSelectorContext(editor) {
	...
	var slateRef = (0, import_react.useRef)({ editor }).current;
	var onChange = (0, import_react.useCallback)((editor) => {
		slateRef.editor = editor;
		eventListeners.forEach((listener) => listener(editor));
	}, []);
	return {
		selectorContext: (0, import_react.useMemo)(() => {
			return {
...
var Slate = (props) => {
	...
	var [context, setContext] = import_react.useState(() => {
		if (!Node.isNodeList(value)) throw new Error("[Slate] value is invalid! Expected a list of elements but got: ".concat(Scrubber.stringify(value)));
		if (!Editor.isEditor(editor)) throw new Error("[Slate] editor is invalid! You passed: ".concat(Scrubber.stringify(editor)));
	...
	return /* @__PURE__ */ import_react.createElement(SlateSelectorContext.Provider, { value: selectorContext }, /* @__PURE__ */ import_react.createElement(SlateContext.Provider, { value: context }, /* @__PURE__ */ import_react.createElement(EditorContext.Provider, { value: context.editor }, /* @__PURE__ */ import_react.createElement(FocusedContext.Provider, { value: isFocused }, children))));
};
/**
* Get the current editor object from the React context.
...
var findCurrentLineRange = (editor, parentRange) => {
	...
	var positions = Array.from(Editor.positions(editor, { at: parentRange }));
	var left = 0;
	var right = positions.length;
	var middle = Math.floor(right / 2);
	if (areRangesSameLine(editor, Editor.range(editor, positions[left]), parentRangeBoundary)) return Editor.range(editor, positions[left], parentRangeBoundary);
	if (positions.length < 2) return Editor.range(editor, positions[positions.length - 1], parentRangeBoundary);
	while (middle !== positions.length && middle !== left) {
		if (areRangesSameLine(editor, Editor.range(editor, positions[middle]), parentRangeBoundary)) right = middle;
		else left = middle;
...
var withReact = function withReact(editor) {
	...
	e.addMark = (key, value) => {
		...
		if (!EDITOR_TO_PENDING_INSERTION_MARKS.get(e) && (_EDITOR_TO_PENDING_DI = EDITOR_TO_PENDING_DIFFS.get(e)) !== null && _EDITOR_TO_PENDING_DI !== void 0 && _EDITOR_TO_PENDING_DI.length) EDITOR_TO_PENDING_INSERTION_MARKS.set(e, null);
		EDITOR_TO_USER_MARKS.delete(e);
		addMark(key, value);
	};
	e.removeMark = (key) => {
		var _EDITOR_TO_PENDING_DI2;
		if (!EDITOR_TO_PENDING_INSERTION_MARKS.get(e) && (_EDITOR_TO_PENDING_DI2 = EDITOR_TO_PENDING_DIFFS.get(e)) !== null && _EDITOR_TO_PENDING_DI2 !== void 0 && _EDITOR_TO_PENDING_DI2.length) EDITOR_TO_PENDING_INSERTION_MARKS.set(e, null);
		EDITOR_TO_USER_MARKS.delete(e);
		removeMark(key);
	};
	e.deleteBackward = (unit) => {
		...
		if (e.selection && Range.isCollapsed(e.selection)) {
			...
			if (parentBlockEntry) {
				...
				var currentLineRange = findCurrentLineRange(e, Editor.range(e, parentBlockPath, e.selection.anchor));
				if (!Range.isCollapsed(currentLineRange)) Transforms.delete(e, { at: currentLineRange });
			}
		}
	};
	e.apply = (op) => {
		...
		if (pendingAction !== null && pendingAction !== void 0 && pendingAction.at) {
			...
			EDITOR_TO_PENDING_ACTION.set(e, at ? _objectSpread(_objectSpread({}, pendingAction), {}, { at }) : null);
		}
		switch (op.type) {
			case "insert_text":
			case "remove_text":
	...
	e.setFragmentData = (data) => {
		...
		var startVoid = Editor.void(e, { at: start.path });
		var endVoid = Editor.void(e, { at: end.path });
		if (Range.isCollapsed(selection) && !startVoid) return;
		...
		Array.from(contents.querySelectorAll("[data-slate-zero-width]")).forEach((zw) => {
			zw.textContent = zw.getAttribute("data-slate-zero-width") === "n" ? "\n" : "";
		});
		if (isDOMText(attach)) {
	...
	e.insertTextData = (data) => {
		...
		if (text) {
			...
			for (var line of lines) {
				if (split) Transforms.splitNodes(e, { always: true });
				e.insertText(line);
				split = true;
			}
			return true;
...
var getMatches = (e, path) => {
	var matches = [];
	for (var [n, p] of Editor.levels(e, { at: path })) {

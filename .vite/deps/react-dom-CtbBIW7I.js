	...
	(function() {
		...
		if (typeof window === "undefined" || typeof MessageChannel !== "function") {
			...
			requestHostCallback = function(cb) {
				if (_callback !== null) setTimeout(requestHostCallback, 0, cb);
				else {
					_callback = cb;
					setTimeout(_flushCallback, 0);
				}
			...
			if (typeof console !== "undefined") {
				...
				if (typeof requestAnimationFrame !== "function") console["error"]("This browser doesn't support requestAnimationFrame. Make sure that you load a polyfill in older browsers. https://reactjs.org/link/react-polyfills");
				if (typeof cancelAnimationFrame !== "function") console["error"]("This browser doesn't support cancelAnimationFrame. Make sure that you load a polyfill in older browsers. https://reactjs.org/link/react-polyfills");
			}
			...
			exports.unstable_forceFrameRate = function(fps) {
				...
				if (fps > 0) yieldInterval = Math.floor(1e3 / fps);
				else yieldInterval = 5;
			};
			var performWorkUntilDeadline = function() {
				if (scheduledHostCallback !== null) {
					...
					try {
						if (!scheduledHostCallback(hasTimeRemaining, currentTime)) {
		...
		function siftDown(heap, node, i) {
			...
			while (index < length) {
				...
				if (left !== void 0 && compare(left, node) < 0) if (right !== void 0 && compare(right, left) < 0) {
					heap[index] = right;
...
//#endregion
//#region node_modules/scheduler/cjs/scheduler-tracing.development.js
/** @license React v0.20.2
...
* LICENSE file in the root directory of this source tree.
*/
var require_scheduler_tracing_development = /* @__PURE__ */ __commonJSMin(((exports) => {
	(function() {
		"use strict";
...
//#endregion
//#region node_modules/scheduler/tracing.js
...
var require_react_dom_development = /* @__PURE__ */ __commonJSMin(((exports) => {
	(function() {
		...
		*/
		...
		function describeFiber(fiber) {
			...
			switch (fiber.tag) {
				...
				case Block: return describeFunctionComponentFrame(fiber.type._render);
		...
		*/
		...
		var voidElementTags = _assign({ menuitem: true }, {
			area: true,
			base: true,
			br: true,
			col: true,
		...
		*/
		...
		function replayUnblockedEvents() {
			...
			while (queuedDiscreteEvents.length > 0) {
				...
				if (nextDiscreteEvent.blockedOn === null) queuedDiscreteEvents.shift();
			}
		...
		*/
		function registerSimplePluginEventsAndSetTheirPriorities(eventTypes, priority) {
			for (var i = 0; i < eventTypes.length; i += 2) {
				...
				var reactName = "on" + (event[0].toUpperCase() + event.slice(1));
		...
		function schedulerPriorityToLanePriority(schedulerPriorityLevel) {
			switch (schedulerPriorityLevel) {
				case ImmediatePriority: return SyncLanePriority;
				case UserBlockingPriority: return InputContinuousLanePriority;
				case NormalPriority:
				case LowPriority: return DefaultLanePriority;
				case IdlePriority: return IdleLanePriority;
				default: return NoLanePriority;
			}
		}
		function lanePriorityToSchedulerPriority(lanePriority) {
			switch (lanePriority) {
				case SyncLanePriority:
				case SyncBatchedLanePriority: return ImmediatePriority;
				case InputDiscreteHydrationLanePriority:
				case InputDiscreteLanePriority:
				case InputContinuousHydrationLanePriority:
				case InputContinuousLanePriority: return UserBlockingPriority;
				case DefaultHydrationLanePriority:
				case DefaultLanePriority:
				case TransitionHydrationPriority:
				case TransitionPriority:
				case SelectiveHydrationLanePriority:
				case RetryLanePriority: return NormalPriority;
				case IdleHydrationLanePriority:
				case IdleLanePriority:
				case OffscreenLanePriority: return IdlePriority;
				case NoLanePriority: return NoPriority;
				default: throw Error("Invalid update priority: " + lanePriority + ". This is a bug in React.");
			}
		}
		...
		function computeExpirationTime(lane, currentTime) {
			...
			if (priority >= InputContinuousLanePriority) return currentTime + 250;
			else if (priority >= TransitionPriority) return currentTime + 5e3;
		...
		function findUpdateLane(lanePriority, wipLanes) {
			switch (lanePriority) {
				case NoLanePriority: break;
				case SyncLanePriority: return SyncLane;
				case SyncBatchedLanePriority: return SyncBatchedLane;
				case InputDiscreteLanePriority:
					var _lane = pickArbitraryLane(InputDiscreteLanes & ~wipLanes);
					if (_lane === NoLane) return findUpdateLane(InputContinuousLanePriority, wipLanes);
					return _lane;
				case InputContinuousLanePriority:
					var _lane2 = pickArbitraryLane(InputContinuousLanes & ~wipLanes);
					if (_lane2 === NoLane) return findUpdateLane(DefaultLanePriority, wipLanes);
					return _lane2;
				case DefaultLanePriority:
					var _lane3 = pickArbitraryLane(DefaultLanes & ~wipLanes);
					if (_lane3 === NoLane) {
						_lane3 = pickArbitraryLane(TransitionLanes & ~wipLanes);
						if (_lane3 === NoLane) _lane3 = pickArbitraryLane(DefaultLanes);
					}
					return _lane3;
				case TransitionPriority:
				case RetryLanePriority: break;
				case IdleLanePriority:
		...
		function findTransitionLane(wipLanes, pendingLanes) {
			...
			if (lane === NoLane) {
				...
				if (lane === NoLane) lane = pickArbitraryLane(TransitionLanes);
			}
			return lane;
		}
		...
		*/
		...
		var SyntheticDragEvent = createSyntheticEvent(_assign({}, MouseEventInterface, { dataTransfer: 0 }));
		var SyntheticFocusEvent = createSyntheticEvent(_assign({}, UIEventInterface, { relatedTarget: 0 }));
		var SyntheticAnimationEvent = createSyntheticEvent(_assign({}, EventInterface, {
			animationName: 0,
			elapsedTime: 0,
			pseudoElement: 0
		}));
		var SyntheticClipboardEvent = createSyntheticEvent(_assign({}, EventInterface, { clipboardData: function(event) {
			return "clipboardData" in event ? event.clipboardData : window.clipboardData;
		} }));
		var SyntheticCompositionEvent = createSyntheticEvent(_assign({}, EventInterface, { data: 0 }));
		/**
		* @interface Event
		...
		*/
		...
		var SyntheticPointerEvent = createSyntheticEvent(_assign({}, MouseEventInterface, {
			pointerId: 0,
			width: 0,
			height: 0,
			pressure: 0,
		...
		var SyntheticTouchEvent = createSyntheticEvent(_assign({}, UIEventInterface, {
			touches: 0,
			targetTouches: 0,
			changedTouches: 0,
			altKey: 0,
		...
		var SyntheticTransitionEvent = createSyntheticEvent(_assign({}, EventInterface, {
			propertyName: 0,
			elapsedTime: 0,
			pseudoElement: 0
		}));
		var SyntheticWheelEvent = createSyntheticEvent(_assign({}, MouseEventInterface, {
		...
		*/
		...
		function diffProperties(domElement, tag, lastRawProps, nextRawProps, rootContainerElement) {
			...
			for (propKey in nextProps) {
				...
				} else if (typeof nextProp === "object" && nextProp !== null && nextProp.$$typeof === REACT_OPAQUE_ID_TYPE) nextProp.toString();
				...
				if (propKey === CHILDREN) {
					...
					if (suppressHydrationWarning);
						...
						if (propertyInfo !== null) {
							...
							if (ownNamespace === HTML_NAMESPACE$1) extraAttributeNames.delete(propKey.toLowerCase());
							else {
		...
		*/
		...
		function didNotHydrateInstance(parentType, parentProps, parentInstance, instance) {
			if (parentProps[SUPPRESS_HYDRATION_WARNING$1] !== true) if (instance.nodeType === ELEMENT_NODE) warnForDeletedHydratableElement(parentInstance, instance);
			else if (instance.nodeType === COMMENT_NODE);
		...
		*/
		...
		function getCurrentPriorityLevel() {
			switch (Scheduler_getCurrentPriorityLevel()) {
				case Scheduler_ImmediatePriority: return ImmediatePriority$1;
				...
				default: throw Error("Unknown priority level.");
			}
		}
		function reactPriorityToSchedulerPriority(reactPriorityLevel) {
			switch (reactPriorityLevel) {
				case ImmediatePriority$1: return Scheduler_ImmediatePriority;
				case UserBlockingPriority$2: return Scheduler_UserBlockingPriority;
				case NormalPriority$1: return Scheduler_NormalPriority;
				case LowPriority$1: return Scheduler_LowPriority;
				case IdlePriority$1: return Scheduler_IdlePriority;
				default: throw Error("Unknown priority level.");
			}
		}
		...
		function checkShouldComponentUpdate(workInProgress, ctor, oldProps, newProps, oldState, newState, nextContext) {
			...
			if (typeof instance.shouldComponentUpdate === "function") {
				...
				if (shouldUpdate === void 0) error("%s.shouldComponentUpdate(): Returned undefined instead of a boolean value. Make sure to return true or false.", getComponentName(ctor) || "Component");
				return shouldUpdate;
			}
		...
		*/
		...
		function ChildReconciler(shouldTrackSideEffects) {
			...
			function updateSlot(returnFiber, oldFiber, newChild, lanes) {
				...
				if (typeof newChild === "object" && newChild !== null) {
					switch (newChild.$$typeof) {
						case REACT_ELEMENT_TYPE: if (newChild.key === key) {
			...
			function updateFromMap(existingChildren, returnFiber, newIdx, newChild, lanes) {
				if (typeof newChild === "string" || typeof newChild === "number") return updateTextNode(returnFiber, existingChildren.get(newIdx) || null, "" + newChild, lanes);
				if (typeof newChild === "object" && newChild !== null) {
					switch (newChild.$$typeof) {
						case REACT_ELEMENT_TYPE:
			...
			*/
			...
			function reconcileChildFibers(returnFiber, currentFirstChild, newChild, lanes) {
				...
				if (typeof newChild === "undefined" && !isUnkeyedTopLevelFragment) switch (returnFiber.tag) {
					case ClassComponent: if (returnFiber.stateNode.render._isMockFunction) break;
					case Block:
					case FunctionComponent:
					case ForwardRef:
					case SimpleMemoComponent: throw Error((getComponentName(returnFiber.type) || "Component") + "(...): Nothing was returned from render. This usually means a return statement is missing. Or, to render nothing, return null.");
				}
		...
		function shouldCaptureSuspense(workInProgress, hasInvisibleParent) {
			...
			if (props.fallback === void 0) return false;
		...
		function checkDepsAreArrayDev(deps) {
			if (deps !== void 0 && deps !== null && !Array.isArray(deps)) error("%s received a final argument that is not an array (instead, received `%s`). When specified, the final argument must be an array.", currentHookNameInDev, typeof deps);
		}
		...
		function readFromUnsubcribedMutableSource(root, source, getSnapshot) {
			...
			if (currentRenderVersion !== null) isSafeToReadFromSource = currentRenderVersion === version;
			else {
				isSafeToReadFromSource = isSubsetOfLanes(renderLanes, root.mutableReadLanes);
				if (isSafeToReadFromSource) setWorkInProgressVersion(source, version);
			}
		...
		function useMutableSource(hook, source, getSnapshot, subscribe) {
			...
			dispatcher.useEffect(function() {
				...
				if (!objectIs(version, getVersion(source._source))) {
					...
					if (!objectIs(snapshot, maybeNewSnapshot)) {
						...
						markRootMutableRead(root, requestUpdateLane(fiber));
					}
		...
		function dispatchAction(fiber, queue, action) {
			...
			var alternate = fiber.alternate;
			if (fiber === currentlyRenderingFiber$1 || alternate !== null && alternate === currentlyRenderingFiber$1) didScheduleRenderPhaseUpdateDuringThisPass = didScheduleRenderPhaseUpdate = true;
			else {
		...
		function updateMemoComponent(current, workInProgress, Component, nextProps, updateLanes, renderLanes) {
			if (current === null) {
				...
				if (innerPropTypes) checkPropTypes(innerPropTypes, nextProps, "prop", getComponentName(type));
			...
			if (_innerPropTypes) checkPropTypes(_innerPropTypes, nextProps, "prop", getComponentName(_type));
		...
		function updateOffscreenComponent(current, workInProgress, renderLanes) {
			...
			if (nextProps.mode === "hidden" || nextProps.mode === "unstable-defer-without-hiding") if ((workInProgress.mode & ConcurrentMode) === NoMode) {
				workInProgress.memoizedState = { baseLanes: NoLanes };
				...
				workInProgress.memoizedState = { baseLanes: nextBaseLanes };
		...
		function markRef(current, workInProgress) {
			var ref = workInProgress.ref;
			if (current === null && ref !== null || current !== null && current.ref !== ref) workInProgress.flags |= Ref;
		}
		...
		function updateSuspenseComponent(current, workInProgress, renderLanes) {
			...
			if ((workInProgress.flags & DidCapture) !== NoFlags || shouldRemainOnFallback(suspenseContext, current)) {
				showFallback = true;
				workInProgress.flags &= ~DidCapture;
			} else if (current === null || current.memoizedState !== null) {
				if (nextProps.fallback !== void 0 && nextProps.unstable_avoidThisFallback !== true) suspenseContext = addSubtreeSuspenseContext(suspenseContext, InvisibleParentSuspenseContext);
			}
			suspenseContext = setDefaultShallowSuspenseContext(suspenseContext);
			pushSuspenseContext(workInProgress, suspenseContext);
			if (current === null) {
				if (nextProps.fallback !== void 0) tryToClaimNextHydratableInstance(workInProgress);
			...
			} else if (current.memoizedState !== null) if (showFallback) {
		...
		function updateSuspenseFallbackChildren(current, workInProgress, primaryChildren, fallbackChildren, renderLanes) {
			...
			if ((mode & BlockingMode) === NoMode && workInProgress.child !== currentPrimaryChildFragment) {
				...
				} else workInProgress.firstEffect = workInProgress.lastEffect = null;
		...
		function propagateSuspenseContextChange(workInProgress, firstChild, renderLanes) {
			...
			while (node !== null) {
				if (node.tag === SuspenseComponent) {
					if (node.memoizedState !== null) scheduleWorkOnFiber(node, renderLanes);
				} else if (node.tag === SuspenseListComponent) scheduleWorkOnFiber(node, renderLanes);
				else if (node.child !== null) {
					node.child.return = node;
					node = node.child;
					continue;
		...
		function validateSuspenseListChildren(children, revealOrder) {
			if ((revealOrder === "forwards" || revealOrder === "backwards") && children !== void 0 && children !== null && children !== false) if (Array.isArray(children)) {
		...
		function updateSuspenseListComponent(current, workInProgress, renderLanes) {
			...
			pushSuspenseContext(workInProgress, suspenseContext);
			if ((workInProgress.mode & BlockingMode) === NoMode) workInProgress.memoizedState = null;
				...
				case "backwards":
					...
					break;
				case "together":
					initSuspenseListRenderState(workInProgress, false, null, null, void 0, workInProgress.lastEffect);
					break;
		...
		function updateContextProvider(current, workInProgress, renderLanes) {
			...
			if (oldProps !== null) {
				...
				} else propagateContextChange(workInProgress, context, changedBits, renderLanes);
			}
		...
		function beginWork(current, workInProgress, renderLanes) {
			...
			switch (workInProgress.tag) {
				...
				case FunctionComponent:
					...
					return updateFunctionComponent(current, workInProgress, _Component, workInProgress.elementType === _Component ? unresolvedProps : resolveDefaultProps(_Component, unresolvedProps), renderLanes);
				case ClassComponent:
					var _Component2 = workInProgress.type;
					var _unresolvedProps = workInProgress.pendingProps;
					return updateClassComponent(current, workInProgress, _Component2, workInProgress.elementType === _Component2 ? _unresolvedProps : resolveDefaultProps(_Component2, _unresolvedProps), renderLanes);
				...
				case SimpleMemoComponent: return updateSimpleMemoComponent(current, workInProgress, workInProgress.type, workInProgress.pendingProps, updateLanes, renderLanes);
				case IncompleteClassComponent:
					var _Component3 = workInProgress.type;
					var _unresolvedProps4 = workInProgress.pendingProps;
					return mountIncompleteClassComponent(current, workInProgress, _Component3, workInProgress.elementType === _Component3 ? _unresolvedProps4 : resolveDefaultProps(_Component3, _unresolvedProps4), renderLanes);
				case SuspenseListComponent: return updateSuspenseListComponent(current, workInProgress, renderLanes);
				case FundamentalComponent: break;
			...
			switch (workInProgress.tag) {
				...
				case HostRoot:
					...
					if (current === null || current.child === null) {
						...
						else if (!fiberRoot.hydrate) workInProgress.flags |= Snapshot;
					}
				...
				case SuspenseComponent:
					...
					if (current === null) {
						if (workInProgress.memoizedProps.fallback !== void 0) popHydrationState(workInProgress);
					} else prevDidTimeout = current.memoizedState !== null;
					if (nextDidTimeout && !prevDidTimeout) {
						if ((workInProgress.mode & BlockingMode) !== NoMode) if (current === null && workInProgress.memoizedProps.unstable_avoidThisFallback !== true || hasSuspenseContext(suspenseStackCursor.current, InvisibleParentSuspenseContext)) renderDidSuspend();
						else renderDidSuspendDelayIfPossible();
					}
					if (nextDidTimeout || prevDidTimeout) workInProgress.flags |= Update;
					return null;
				case HostPortal:
					popHostContainer(workInProgress);
					updateHostContainer(workInProgress);
				...
				case LegacyHiddenComponent:
					...
					if (current !== null) {
						...
						if (current.memoizedState !== null !== (_nextState !== null) && newProps.mode !== "unstable-defer-without-hiding") workInProgress.flags |= Update;
					}
					return null;
			}
			throw Error("Unknown unit of work tag (" + workInProgress.tag + "). This error is likely caused by a bug in React. Please file an issue.");
		...
		function throwException(root, returnFiber, sourceFiber, value, rootRenderLanes) {
			...
			if (value !== null && typeof value === "object" && typeof value.then === "function") {
				...
				value = /* @__PURE__ */ new Error((getComponentName(sourceFiber.type) || "A React component") + " suspended while rendering, but no fallback UI was specified.\n\nAdd a <Suspense fallback=...> component higher in the tree to provide a loading indicator or placeholder to display.");
			}
			renderDidError();
		...
		function safelyCallComponentWillUnmount(current, instance) {
			...
			if (hasCaughtError()) captureCommitPhaseError(current, clearCaughtError());
		}
		...
		function commitDetachRef(current) {
			...
			if (currentRef !== null) if (typeof currentRef === "function") currentRef(null);
					...
					if (updateQueue !== null) {
						...
						if (lastEffect !== null) {
							...
							do {
								...
								if (destroy !== void 0) if ((tag & Passive$1) !== NoFlags$1) enqueuePendingPassiveHookEffectUnmount(current, effect);
		...
		function requestUpdateLane(fiber) {
			...
			else lane = findUpdateLane(schedulerPriorityToLanePriority(schedulerPriority), currentEventWipLanes);
			return lane;
		}
		...
		function scheduleUpdateOnFiber(fiber, lane, eventTime) {
			...
			var priorityLevel = getCurrentPriorityLevel();
			if (lane === SyncLane) if ((executionContext & LegacyUnbatchedContext) !== NoContext && (executionContext & (RenderContext | CommitContext)) === NoContext) {
				...
				if ((executionContext & DiscreteEventContext) !== NoContext && (priorityLevel === UserBlockingPriority$2 || priorityLevel === ImmediatePriority$1)) if (rootsWithPendingDiscreteUpdates === null) rootsWithPendingDiscreteUpdates = new Set([root]);
		...
		function ensureRootIsScheduled(root, currentTime) {
			...
			var newCallbackNode;
			if (newCallbackPriority === SyncLanePriority) newCallbackNode = scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root));
			...
			else newCallbackNode = scheduleCallback(lanePriorityToSchedulerPriority(newCallbackPriority), performConcurrentWorkOnRoot.bind(null, root));
		...
		function renderDidSuspendDelayIfPossible() {
			...
			if (workInProgressRoot !== null && (includesNonIdleWork(workInProgressRootSkippedLanes) || includesNonIdleWork(workInProgressRootUpdatedLanes))) markRootSuspended$1(workInProgressRoot, workInProgressRootRenderLanes);
		}
		...
		function resetChildLanes(completedWork) {
			if ((completedWork.tag === LegacyHiddenComponent || completedWork.tag === OffscreenComponent) && completedWork.memoizedState !== null && !includesSomeLane(subtreeRenderLanes, OffscreenLane) && (completedWork.mode & ConcurrentMode) !== NoLanes) return;
			...
			if ((completedWork.mode & ProfileMode) !== NoMode) {
				...
				if (completedWork.tag === SuspenseComponent && completedWork.memoizedState !== null) {
		...
		function commitRootImpl(root, renderPriorityLevel) {
			...
			if (!rootDidHavePassiveEffects) finishPendingInteractions(root, lanes);
			if (remainingLanes === SyncLane) if (root === rootWithNestedUpdates) nestedUpdateCount++;
			else {
				nestedUpdateCount = 0;
		...
		function commitBeforeMutationEffects() {
			while (nextEffect !== null) {
				...
				if (!shouldFireAfterActiveInstanceBlur && focusedInstanceHandle !== null) {
					...
					} else if (nextEffect.tag === SuspenseComponent && isSuspenseBoundaryBeingHidden(current, nextEffect) && doesFiberContain(nextEffect, focusedInstanceHandle)) shouldFireAfterActiveInstanceBlur = true;
				}
		...
		function commitMutationEffects(root, renderPriorityLevel) {
			while (nextEffect !== null) {
				...
				switch (flags & (Placement | Update | Deletion | Hydrating)) {
					case Placement:
				...
				if (fiber.tag === HostRoot) {
					...
					if (typeof ctor.getDerivedStateFromError === "function" || typeof instance.componentDidCatch === "function" && !isAlreadyFailedLegacyErrorBoundary(instance)) {
						...
						if (root !== null) {
							...
							ensureRootIsScheduled(root, eventTime);
							schedulePendingInteractions(root, SyncLane);
						} else if (typeof instance.componentDidCatch === "function" && !isAlreadyFailedLegacyErrorBoundary(instance)) try {
		...
		function warnAboutUpdateOnNotYetMountedFiberInDEV(fiber) {
			...
			var tag = fiber.tag;
			if (tag !== IndeterminateComponent && tag !== HostRoot && tag !== ClassComponent && tag !== FunctionComponent && tag !== ForwardRef && tag !== MemoComponent && tag !== SimpleMemoComponent && tag !== Block) return;
		...
		function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
			var tag = fiber.tag;
			if (tag !== HostRoot && tag !== ClassComponent && tag !== FunctionComponent && tag !== ForwardRef && tag !== MemoComponent && tag !== SimpleMemoComponent && tag !== Block) return;
		...
		beginWork$1 = function(current, unitOfWork, lanes) {
			...
			try {
				...
				if (hasCaughtError()) throw clearCaughtError();
			...
			if (isRendering && (executionContext & RenderContext) !== NoContext && !getIsUpdatingOpaqueValueInRenderPhaseInDEV()) switch (fiber.tag) {
				...
				case SimpleMemoComponent:
					...
					if (!didWarnAboutUpdateInRenderForAnotherComponent.has(dedupeKey)) {
						...
						error("Cannot update a component (`%s`) while rendering a different component (`%s`). To locate the bad setState() call inside `%s`, follow the stack trace as described in https://reactjs.org/link/setstate-in-render", getComponentName(fiber.type) || "Unknown", renderingComponentName, renderingComponentName);
					}
					break;
				case ClassComponent:
		...
		function markSpawnedWork(lane) {
			if (spawnedWorkDuringRender === null) spawnedWorkDuringRender = [lane];
		...
		function createFiberFromTypeAndProps(type, key, pendingProps, owner, mode, lanes) {
			...
			if (typeof type === "function") if (shouldConstruct$1(type)) {
				...
				case REACT_LEGACY_HIDDEN_TYPE: return createFiberFromLegacyHidden(pendingProps, mode, lanes, key);
		...
		function findHostInstanceWithWarning(component, methodName) {
			var fiber = get(component);
			if (fiber === void 0) if (typeof component.render === "function") throw Error("Unable to find node on an unmounted component.");
		...
		function legacyCreateRootFromDOMContainer(container, forceHydrate) {
			...
			return createLegacyRoot(container, shouldHydrate ? { hydrate: true } : void 0);
		}
		...
		function hydrate(element, container, callback) {
			...
			if (isContainerMarkedAsRoot(container) && container._reactRootContainer === void 0) error("You are calling ReactDOM.hydrate() on a container that was previously passed to ReactDOM.createRoot(). This is not supported. Did you mean to call createRoot(container, {hydrate: true}).render(element)?");
			return legacyRenderSubtreeIntoContainer(null, element, container, true, callback);
		}
		function render(element, container, callback) {
			if (!isValidContainer(container)) throw Error("Target container is not a DOM element.");
			if (isContainerMarkedAsRoot(container) && container._reactRootContainer === void 0) error("You are calling ReactDOM.render() on a container that was previously passed to ReactDOM.createRoot(). This is not supported. Did you mean to call root.render(element)?");
		...
		function unmountComponentAtNode(container) {
			...
			if (isContainerMarkedAsRoot(container) && container._reactRootContainer === void 0) error("You are calling ReactDOM.unmountComponentAtNode() on a container that was previously passed to ReactDOM.createRoot(). This is not supported. Did you mean to call root.unmount()?");
...
//# sourceMappingURL=react-dom-CtbBIW7I.js.map
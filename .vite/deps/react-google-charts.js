...
//#region node_modules/react-google-charts/dist/index.js
...
*/ function useLoadScript(src, onLoad, onError) {
	(0, import_react.useEffect)(() => {
		...
		if (!foundScript) document.head.append(script);
	}, []);
}
/**
...
*/ function useLoadGoogleCharts(param) {
	let { chartVersion = "current", chartPackages = ["corechart", "controls"], chartLanguage = "en", mapsApiKey } = param;
...
*/ function LoadGoogleCharts(param) {
	...
	(0, import_react.useEffect)(() => {
		if (googleCharts && onLoad) onLoad(googleCharts);
	}, [googleCharts]);
	(0, import_react.useEffect)(() => {
...
var loadDataTableFromSpreadSheet = async function(googleViz, spreadSheetUrl) {
	...
	return new Promise((resolve, reject) => {
		...
		new googleViz.visualization.Query(urlToSpreadSheet).send((response) => {
			if (response.isError()) reject("Error in query:  ".concat(response.getMessage(), " ").concat(response.getDetailedMessage()));
			else resolve(response.getDataTable());
		});
	});
};
var { Provider, Consumer } = /* @__PURE__ */ import_react.createContext(chartDefaultProps);
var ContextProvider = (param) => {
	let { children, value } = param;
	return /* @__PURE__ */ import_react.createElement(Provider, { value }, children);
};
var ContextConsumer = (param) => {
	let { render } = param;
	return /* @__PURE__ */ import_react.createElement(Consumer, null, (context) => {
		return render(context);
	});
};
var GRAY_COLOR = "#CCCCCC";
var GoogleChartDataTableInner = class extends import_react.Component {
	componentDidMount() {
		this.draw(this.props);
		window.addEventListener("resize", this.onResize);
		if (this.props.legend_toggle || this.props.legendToggle) this.listenToLegendToggle();
	}
	componentWillUnmount() {
		const { google, googleChartWrapper } = this.props;
		window.removeEventListener("resize", this.onResize);
		google.visualization.events.removeAllListeners(googleChartWrapper);
		if (googleChartWrapper.getChartType() === "Timeline") googleChartWrapper.getChart() && googleChartWrapper.getChart().clearChart();
	}
	componentDidUpdate() {
		this.draw(this.props);
	}
	...
	constructor(...args) {
		...
		this.listenToLegendToggle = () => {
			...
			google.visualization.events.addListener(googleChartWrapper, "select", () => {
				const selection = googleChartWrapper.getChart().getSelection();
				const dataTable = googleChartWrapper.getDataTable();
				if (selection.length === 0 || selection[0].row || !dataTable) return;
				const columnIndex = selection[0].column;
				const columnID = this.getColumnID(dataTable, columnIndex);
				if (this.state.hiddenColumns.includes(columnID)) this.setState((state) => ({
					...state,
		...
		this.applyFormatters = (dataTable, formatters) => {
			...
			for (let formatter of formatters) switch (formatter.type) {
				case "ArrowFormat":
					new google.visualization.ArrowFormat(formatter.options).format(dataTable, formatter.column);
					break;
				case "BarFormat":
					new google.visualization.BarFormat(formatter.options).format(dataTable, formatter.column);
					break;
				case "ColorFormat": {
					const vizFormatter = new google.visualization.ColorFormat(formatter.options);
					const { ranges } = formatter;
					for (let range of ranges) vizFormatter.addRange(...range);
					vizFormatter.format(dataTable, formatter.column);
					break;
				}
				case "DateFormat":
					new google.visualization.DateFormat(formatter.options).format(dataTable, formatter.column);
					break;
				case "NumberFormat":
					new google.visualization.NumberFormat(formatter.options).format(dataTable, formatter.column);
					break;
				case "PatternFormat":
					new google.visualization.PatternFormat(formatter.options).format(dataTable, formatter.column);
					break;
			}
		};
		...
		this.draw = async (param) => {
			...
			if (data) if (Array.isArray(data)) dataTable = google.visualization.arrayToDataTable(data);
			else dataTable = new google.visualization.DataTable(data);
			else if (rows && columns) dataTable = google.visualization.arrayToDataTable([columns, ...rows]);
			else if (spreadSheetUrl) dataTable = await loadDataTableFromSpreadSheet(google, spreadSheetUrl, spreadSheetQueryParameters);
			else dataTable = google.visualization.arrayToDataTable([]);
			const columnCount = dataTable.getNumberOfColumns();
			const viewColumns = Array(columnCount).fill(0).map((c, i) => {
				const columnID = this.getColumnID(dataTable, i);
				if (this.state.hiddenColumns.includes(columnID)) return {
			...
			if (googleChartWrapper.getChartType() === "Timeline") chart && chart.clearChart();
			...
			if (this.props.googleChartDashboard !== null) this.props.googleChartDashboard.draw(dataTable);
			if (chartDiff) {
			...
			if (legendToggle === true || legend_toggle === true) this.grayOutHiddenColumns({ options });
		};
		this.grayOutHiddenColumns = (param) => {
			...
			if (!dataTable) return;
			const columnCount = dataTable.getNumberOfColumns();
			if (this.state.hiddenColumns.length > 0 === false) return;
			const colors = Array.from({ length: columnCount - 1 }).map((dontcare, i) => {
				const columnID = this.getColumnID(dataTable, i + 1);
				if (this.state.hiddenColumns.includes(columnID)) return GRAY_COLOR;
				else if (options && options.colors) return options.colors[i];
				else return DEFAULT_CHART_COLORS[i];
			});
...
var GoogleChartDataTable = class extends import_react.Component {
	componentDidMount() {}
	componentWillUnmount() {}
	...
	render() {
		...
		return /* @__PURE__ */ import_react.createElement(ContextConsumer, { render: (props) => {
			return /* @__PURE__ */ import_react.createElement(GoogleChartDataTableInner, Object.assign({}, props, {
				google,
				googleChartWrapper,
...
var GoogleChartEvents = class extends import_react.Component {
	shouldComponentUpdate() {
		return false;
	}
	listenToEvents(param) {
		let { chartEvents, google, googleChartWrapper } = param;
		if (!chartEvents) return;
		google.visualization.events.removeAllListeners(googleChartWrapper);
	...
	render() {
		this.props;
		return /* @__PURE__ */ import_react.createElement(ContextConsumer, { render: (propsFromContext) => {
...
var GoogleChart = class extends import_react.Component {
	componentDidMount() {
		...
		googleChartWrapper.setOptions(options || {});
		if (getChartWrapper) getChartWrapper(googleChartWrapper, google);
		...
		if (toolbarItems) google.visualization.drawToolbar(this.toolbar_ref.current, toolbarItems);
	...
	componentDidUpdate() {
		...
		if (controls) for (let i = 0; i < controls.length; i += 1) {
			const { controlType, options, controlWrapperParams } = controls[i];
			if (controlWrapperParams && "state" in controlWrapperParams) this.state.googleChartControls[i].control.setState(controlWrapperParams["state"]);
	...
	render() {
		...
		if (this.props.render) return /* @__PURE__ */ import_react.createElement("div", {
		...
		else return /* @__PURE__ */ import_react.createElement("div", {
			ref: this.dashboard_ref,
			style: divStyle
	...
	constructor(...args1) {
		...
		this.graphID = null;
		this.dashboard_ref = /* @__PURE__ */ import_react.createRef();
		this.toolbar_ref = /* @__PURE__ */ import_react.createRef();
		this.getGraphID = () => {
			const { graphID, graph_id } = this.props;
			let instanceGraphID;
			if (!graphID && !graph_id) if (!this.graphID) instanceGraphID = generateUniqueID();
			else instanceGraphID = this.graphID;
			else if (graphID && !graph_id) instanceGraphID = graphID;
			else if (graph_id && !graphID) instanceGraphID = graph_id;
			else instanceGraphID = graphID;
		...
		this.getControlID = (id, index) => {
			controlCounter += 1;
			let controlID;
			if (typeof id === "undefined") controlID = "googlechart-control-".concat(index, "-").concat(controlCounter);
			else controlID = id;
			return controlID;
		};
		this.addControls = (googleChartWrapper, googleChartDashboard) => {
			...
			if (!googleChartControls) return null;
		...
		this.renderChart = () => {
			...
			return /* @__PURE__ */ import_react.createElement("div", Object.assign({
				...
				style: divStyle,
				className
			}, rootProps), this.state.isReady && this.state.googleChartWrapper !== null ? /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, /* @__PURE__ */ import_react.createElement(GoogleChartDataTable, {
				googleChartWrapper: this.state.googleChartWrapper,
				google,
				googleChartDashboard: this.state.googleChartDashboard
			}), /* @__PURE__ */ import_react.createElement(GoogleChartEvents, {
				googleChartWrapper: this.state.googleChartWrapper,
				google
			})) : null);
		};
		this.renderControl = function() {
			...
			return _this1.state.isReady && _this1.state.googleChartControls !== null ? /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, _this1.state.googleChartControls.filter((param) => {
		...
		this.renderToolBar = () => {
			...
			return /* @__PURE__ */ import_react.createElement("div", { ref: this.toolbar_ref });
		};
	}
};
var Chart$1 = class extends import_react.Component {
	render() {
		const { chartLanguage, chartPackages, chartVersion, mapsApiKey, loader, errorElement } = this.props;
		return /* @__PURE__ */ import_react.createElement(ContextProvider, { value: this.props }, this.state.loadingStatus === "ready" && this.state.google !== null ? /* @__PURE__ */ import_react.createElement(GoogleChart, Object.assign({}, this.props, { google: this.state.google })) : this.state.loadingStatus === "errored" && errorElement ? errorElement : loader, /* @__PURE__ */ import_react.createElement(LoadGoogleCharts, {
			chartLanguage,
			chartPackages,
			chartVersion,
			mapsApiKey,
	...
	constructor(...args) {
		...
		this.onLoad = (google1) => {
			if (this.props.onLoad) this.props.onLoad(google1);
			if (this.isFullyLoaded(google1)) this.onSuccess(google1);
			else {

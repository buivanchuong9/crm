...
export default function BusinessProcessList() {
  ...
  const [isRegimeKanban, setIsRegimeKanban] = useState<boolean>(parseLocalStorageJson<boolean>(checkIsKanban, false));
  useEffect(() => {
    localStorage.setItem("isKanbanBusinessProcess", JSON.stringify(isRegimeKanban));
  }, [isRegimeKanban]);

  const [processId, setProcessId] = useState<number>(parseLocalStorageJson<number>(checkProcessId, -1));

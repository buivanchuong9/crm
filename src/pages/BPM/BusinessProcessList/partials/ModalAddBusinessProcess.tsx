...
import ModalAddStates from "./ModalAddStates/ModalAddStates";
...
export default function ModalAddBusinessProcess(props: any) {
  ...
  const [listStates, setListStates] = useState<IOption[]>([]);
  const [detailObjectType, setDetailObjectType] = useState(null);
  const [typeNodeChange, setTypeNodeChange] = useState(false);
  const [showEditStates, setShowEditStates] = useState<boolean>(false);

  const SLA_TYPE = "SLA";
  const {
    typeNode,
    setTypeNode,
    setIsSubmit: setIsSubmitSla,
    isLoadingType,
    dataConfigAdvance,
    formDataAdvance,
  ...
  const loadOptionStates = async (search, loadedOptions, { page }) => {
    const param: any = {
      keyword: search,
      page: page,
      limit: 10,
  ...
  // Fetch state name by status
  const getStateNameByStatus = async (status: string | number) => {
    if (status === null || status === undefined) {
      console.warn("Status is null or undefined, returning default values");
      return { stateCode: "", stateName: "" };
    }
    const param: any = {
      stateCode: String(status), // Convert to string to handle stateCode = 0
      page: 1,
      limit: 1,
    };
    try {
      const response = await BusinessProcessService.listStates(param);
      if (response.code === 0 && response.result && response.result.items && response.result.items.length > 0) {
        const state = response.result.items[0];
        return { stateCode: state.stateCode, stateName: state.stateName };
      }
      console.warn(`No state found for stateCode: ${status}`);
      return { stateCode: String(status), stateName: String(status) }; // Fallback to status if not found
    } catch (error) {
      console.error(`Lỗi khi lấy stateName cho stateCode: ${status}`, error);
      return { stateCode: String(status), stateName: String(status) }; // Fallback to status if error
    }
  };

  // Fetch process steps and resolve state names
    ...
    if (response.code === 0) {
      ...
      const updatedItems = await Promise.all(
        result.items.map(async (item) => {
          const stateInfo = await getStateNameByStatus(item.stateCode ?? item.status ?? "");
          return {
            ...item,
            checkName: !item.stepName,
            checkState: !stateInfo.stateCode && stateInfo.stateCode !== 0 && stateInfo.stateCode !== "0",
            stateCode: stateInfo.stateCode ?? "",
            stateName: stateInfo.stateName ?? "",
          };
        })
      );
      setListStepProcess(updatedItems);
    } else {
  ...
  const actions = useMemo<IActionModal>(
    ...
    [formData, values, isSubmit, tabStep, processId, valueSLA, valuesSLA, detailObjectType, typeNode, dataConfigAdvanceEdit, formDataAdvance, typeNodeChange]
  );
  ...
  const handleAddStep = async (step: number) => {
    const body: any = {
      stepName: "", // Khởi tạo rỗng
      stepNumber: step,
      processId: processId,
      stateCode: "",
      stateName: "",
      checkName: false, // Không hiển thị lỗi xác thực ban đầu
      checkState: false, // Không hiển thị lỗi xác thực ban đầu
    };

    try {
      // Gọi API để lưu bước mới
      const response = await BusinessProcessService.updateStep(body);
      if (response.code === 0) {
        // Thêm bước mới vào danh sách với ID từ server
        setListStepProcess((current) => [
          ...current,
          { ...body, id: response.result.id },
        ]);
        showToast("Thêm bước quy trình thành công", "success");
      } else {
        showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
      }
    } catch (err) {
      showToast("Có lỗi xảy ra khi thêm bước", "error");
    }
  };

  const handleBlurValueStep = (e, idx) => {
    const value = e.target.value.trim();

    setListStepProcess((current) => {
      const newList = current.map((obj, index) => {
        if (index === idx) {
          const isEmptyState = !(obj.stateCode || obj.stateCode === 0 || obj.stateCode === "0");
          const updatedObj = {
            ...obj,
            stepName: value,
            checkName: !value, // Chỉ hiển thị lỗi nếu rỗng
            checkState: isEmptyState, // Chỉ hiển thị lỗi nếu trạng thái rỗng
          };

          // Chỉ cập nhật bước nếu cả hai trường hợp lệ
  ...
  const handleChangeValueStep = (e, idx) => {
    const value = e.target.value;
    setListStepProcess((current) =>
      current.map((obj, index) => {
        if (index === idx) {
          return { ...obj, stepName: value, checkName: !value ? true : false };
        }
        return obj;
      })
    );
      ...
      const newList = current.map((obj, index) => {
        if (index === idx) {
          const updatedObj = {
            ...
            stateCode: value,
            stateName: label,
            checkState: false, // Xóa lỗi xác thực trạng thái
          };

          // Chỉ cập nhật bước nếu cả hai trường hợp lệ
          if (updatedObj.stepName) {
            updateStep(updatedObj);
          }
  ...
  const updateStep = async (item: any) => {
    try {
      const response = await BusinessProcessService.updateStep({
        ...item,
        status: item.stateCode ?? "",
      });

      if (response.code === 0) {
        showToast("Cập nhật bước quy trình thành công", "success");
        // Cập nhật ID bước nếu cần
        setListStepProcess((current) =>
          current.map((obj) =>
            obj.stepNumber === item.stepNumber ? { ...obj, id: response.result.id } : obj
          )
        );
      } else {
        showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
      ...
      showToast("Có lỗi xảy ra khi cập nhật bước", "error");
    }
  };
  ...
  const handleRemoveStep = (id, idx) => {
    ...
    if (newData.length > 0) {
      newData.map((item) => {
        const promise = new Promise((resolve, reject) => {
          BusinessProcessService.updateStep({
            ...item,
            status: item.stateCode ?? ""
  ...
  const updateSlaAdvanced = async () => {
    const dataConfig = convertDataRow(dataConfigAdvanceEdit, processId);
    const body = {
      id: formDataAdvance.id,
      processId: processId,
      name: SLA_TYPE + "_" + processId,
      type: SLA_TYPE,
      description: SLA_TYPE + "_" + processId,
    ...
    if (response.code === 0) {
      showToast(`Cài đặt ${SLA_TYPE} thành công`, "success");
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  };
  ...
  const onSubmitSLA = async () => {
    ...
    if (typeNode === "advanced") {
      await updateSlaAdvanced();
    } else {
      await updateSlaBasic();
    }
    setIsSubmitSla(false);
    clearForm(true, true);
  };
  ...
  const getDecimalScale = (attributes) => {
    ...
    if (numberFormat.endsWith(".#")) return 1;
    if (numberFormat.endsWith(".##")) return 2;
    if (numberFormat.endsWith(".###")) return 3;
    return 0;
  };
            ...
            <div style={{ display: "flex", margin: "12px 0 0 12px" }}>
              {dataStep.map((item, index) => (
                <div
                  key={index}
                  style={{
                    borderBottom: tabStep === item.value ? "1px solid #CE182D" : "1px solid #EEEEEF",
                    paddingLeft: 12,
                    paddingRight: 12,
                    paddingBottom: 3,
                    cursor: "pointer",
                  ...
                  onClick={() => {
                    if (processId) {
                      ...
                      if (item.value === 2) {
                        getListStepProcess(processId); // Ensure data is refreshed when tab is opened
                      }
                    }
                  }}
                >
                  <span style={{ fontSize: 15, fontWeight: "500", color: tabStep === item.value ? "#ED1B34" : "#939394" }}>{item.label}</span>
                </div>
              ))}
            </div>
            {tabStep === 1 && (
            ...
            {tabStep === 2 && (
              <div className="tab_2_step_process">
                <div style={{ marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <div className="group_button_add_approach">
                    <div
                    ...
                    {/* <div
                      className="button_add_approach"
                      onClick={() => setShowEditStates(true)}
                    >
                      <Icon name="Edit" />
                      <span className="title_button">Chỉnh sửa trạng thái</span>
                    </div> */}
                  </div>
                </div>
                    ...
                    <div key={index} className="container_step">
                      <div className="item_step">
                        <Input
                          ...
                          error={item.checkName} // Chỉ hiển thị lỗi nếu checkName là true
                        ...
                        <SelectCustom
                          ...
                          options={[]}
                          value={
                            item.stateCode || item.stateCode === 0 || item.stateCode === "0"
                              ? { value: item.stateCode, label: item.stateName || item.stateCode }
                              : null
                          }
                          ...
                          additional={{ page: 1 }}
                          loadOptionsPaginate={loadOptionStates}
                          error={item.checkState} // Chỉ hiển thị lỗi nếu checkState là true
                          message="Trạng thái không được để trống"
                        />
                      </div>
                      {listStepProcess.length > 1 && item.id && (
                        <Tippy content="Xóa">
            ...
            {tabStep === 3 && (
              <div>
                <SettingSLA
                  ...
                  setTypeNodeChange={setTypeNodeChange}
                />
              </div>
            )}
            {tabStep === 4 && (

...
export default function ModalExport(props: ExportModalProps) {
  const { name, onShow, onHide, options, callback, type = "one_option", listColumn, listData, typeNo } = props;
  ...
  const createAndDownloadExcel = async () => {    
    ...
    const data = listData.map((item, index) => {
      if (item.key) {
        ...
        listColumn.map((column) => {
          if (column.key === "stt") {
            obj[column.name] = typeNo == "auto" ? index + 1 : item.stt;
          } else {
        ...
        let newRow = {};
        listColumn.forEach((column, indexCol) => {
          newRow[column.name] = indexCol === 0 ? (typeNo == "auto" ? index + 1 : item.stt) : indexCol === 1 ? item.content : "";
        });
        return newRow;
      }
    });
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      // listColumn.map((item) => item.type), // Dòng thứ 3 là kiểu dữ liệu
      // listColumn.map((item) => item?.format_lookup || ""), // Dòng thứ 4 bỏ trống dành cho lookup
    ]);

    console.log("NNNNNNNNNNNNNNNNNNNNNNNNN", listColumnExport);

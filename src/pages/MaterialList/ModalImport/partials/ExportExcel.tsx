...
import { styles, columnFormat, columnWidth, formats  } from "exports/config";
...
export async function ExportExcel(userOptions, product_store?: any, name?: string) {
  const options = {
    title: "",
    titleExtra: [],
    titleExtraFormat: [],
  ...
  const defaultInfo =
    options.generateInfo && options.isShowHeader
      ? [
  ...
  if (options.header && options.header.length > 0) {
    ...
    const headerRows = createHeaderFooter(worksheet, curRowIdx, options.header, options.headerFormat, options.formatExcel, true, true, true);
    curRowIdx += headerRows.length;
  }

  const headerExtraRows = createHeaderFooter(worksheet, curRowIdx, options.headerExtra, [], options.formatExcel, true, true, true, true, true);
  ...
  if (options.isFormImport) {
    ...
    const errMSG = {
      ...
      name: {
        error: "", // đoạn này dùng để thông báo tên không hợp lệ khi import file
      },
      currentCost: {
        error: "", // đoạn này dùng để thông báo giá bán không hợp lệ khi import file
      },
    };
    validation = {
      category: {
  ...
  options.data.forEach((item, idx) => {
    ...
    curRowIdx++;
    let isDQG = false;
    if (options.isFormImport && row.values.length && row.values[1].toLowerCase().indexOf("dqg") > -1) isDQG = true;
    ...
    row.eachCell((cell, index) => {
      ...
      //TODO: đoạn này format style đang chưa hoạt động tạm ẩn
  ...
  // add row to create new data (formImport)
  if (options.isFormImport) {
    [...Array(2000)].forEach(() => {
      const row = worksheetAddRow(
        worksheet,
  ...
  const dataExtraRows = createHeaderFooter(worksheet, curRowIdx, options.dataExtra, [], options.formatExcel, true, true, true, true, true);
  ...
  if (options.isFormImport) {
    ...
    worksheet.getCell("E3").value = "Lưu ý: để thêm thuốc mới bạn bắt buộc điền vào những mục có dấu (*)";

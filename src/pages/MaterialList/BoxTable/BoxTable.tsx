            ...
            <tr>
              {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                <th className="checkbox">
                  <Checkbox
                    indeterminate={listIdChecked?.length > 0 && listIdChecked?.length < items.filter((el) => !el.used).length}
                    checked={listIdChecked?.length > 0 && listIdChecked?.length === items.filter((el) => !el.used).length}
                    ...
                    {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                      <td className="checkbox" onClick={(e) => e.stopPropagation()}>
                        <Checkbox
                          ...
                          disabled={!item?.raw?.used ? false : true}
                        />
                      </td>
                    )}

...
import TableDepartment from "./partials/TableDepartment/TableDepartment";
import AddDepartmentDirectoryModal from "./partials/AddDepartmentDirectoryModal/AddDepartmentDirectoryModal";
import ViewConfigDepartment from "./partials/ViewConfigDepartment/ViewConfigDepartment";
...
import "./DepartmentDirectoryList.scss";
...
export default function DepartmentDirectoryList(props: IDepartmentDirectoryListProps) {
  ...
  const [listDepartment, setListDepartment] = useState<IDepartmentResponse[]>([]);
  const [dataDepartment, setDataDepartment] = useState<IDepartmentResponse>(null);
  const [idDepartment, setIdDepartment] = useState<number>(null);
  ...
  const listTabs = [
    ...
    {
      title: "Sơ đồ phân cấp phòng ban",
      is_active: "tab_two",
      type: 2,
    },
  ];
  ...
  const getListDepartment = async (paramsSearch: IDepartmentFilterRequest) => {
    ...
    const response = await DepartmentService.list(paramsSearch, abortController.signal);
  ...
  const titles = ["STT", "Tên phòng ban", "Phân quyền", "Xem chức danh", "Xem nhân viên", "Trạng thái hoạt động", "Ghi chú"];
  ...
  const [showModalUnitPrice, setShowModalUnitPrice] = useState<boolean>(false);

  const dataMappingArray = (item: IDepartmentResponse, index: number) => [
  ...
  return (
    <div className={`page-content page-department${isNoItem ? " bg-white" : ""}`}>
      ...
      <AddUnitPriceDepartmentModal
        onShow={showModalUnitPrice}
        idDepartment={dataDepartment?.id}
        onHide={(reload) => {
          if (reload) {
            getListDepartment(params);
          }

          setShowModalUnitPrice(false);
        }}
      />

...
export default function AddDepartmentDirectoryModal(props: AddDepartmentModalProps) {
  ...
  const getDetailDepartment = async () => {
    ...
    if (response.code === 0) {
      ...
      if (response.result?.branchId) {
        getDetailBranch(response.result?.branchId);
      }
    } else {
  ...
  const getDetailBranch = async (branchId) => {
    const response = await BeautyBranchService.detail(branchId);

    if (response.code === 0) {
      const result = response.result;
      setBranchData({ value: result?.id, label: result?.name });
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  };
  ...
  const handleChangeBranch = (e) => {
    setBranchData(e);
    setFormData({ ...formData, values: { ...formData?.values, branchId: e.value } });
  };
  ...
  const values = useMemo(
    () =>
      ({
        ...
        branchId: checkUserRoot == "1" ? detailDepartment?.branchId : 0,
  ...
  const listFieldInfoBasic = useMemo(
    ...
    [listDepartmnent, isLoadingDepartmnent, listEmployee, isLoadingEmployee, branchData]
  );

...
export default function ModalAddField(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await FieldListService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} lĩnh vực thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        size="sm"
      >
        <form className="form-add-field-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} lĩnh vực`}
            toggle={() => {

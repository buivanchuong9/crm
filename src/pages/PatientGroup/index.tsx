...
import AddPatientGroup from "./partials/AddPatientGroup";
import ChangeStatusPatientGroup from "./partials/ChangeStatusPatientGroup";
import "./index.scss";
...
export default function PatientGroup() {
  ...
  const [listPatientGroup, setListPatientGroup] = useState([]);
  const [dataPatientGroup, setDataPatientGroup] = useState(null);
  ...
  const getListPatientGroup = async (paramsSearch) => {
    setIsLoading(true);
    ...
    if (response.code === 0) {
      ...
      setListPatientGroup(result.items);
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listPatientGroup.find((item) => item.id === selectedId);
      if (found?.id) {
    ...
    <div className={`page-content page__customer--segment${isNoItem ? " bg-white" : ""}`}>
        ...
        <div className="action-backup">
          ...
          {showModalAdd && (
            <Fragment>
              ...
              />
              <h1 className="title-last">{isViewModalAdd ? "Xem chi tiết" : dataPatientGroup ? "Chỉnh sửa" : "Thêm mới"}</h1>
            </Fragment>
          )}
        </div>
      ...
      <div className="card-box d-flex flex-column">
        <div className={`${showModalAdd ? "d-none" : ""}`}>
          ...
          />
          {!isLoading && listPatientGroup && listPatientGroup.length > 0 ? (
            <BoxTable
              name="Phân khúc khách hàng"
              titles={titles}
              items={listPatientGroup}
              isPagination={true}

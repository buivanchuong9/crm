...
import AddPostExamSurvey from "./partials/AddPostExamSurvey";
import ViewResultSurvey from "./partials/ViewResultSurvey";
import "./index.scss";

export default function PostExamSurvey() {
  document.title = "Khảo sát khách hàng";

  const isMounted = useRef(false);

  const [listPostExamSurvey, setListPostExamSurvey] = useState([]);
  const [dataPostExamSurvey, setDataPostExamSurvey] = useState(null);
  ...
  const getLstPostExamSurvey = async (paramsSearch) => {
    setIsLoading(true);
    ...
    if (response.code === 0) {
      ...
      setListPostExamSurvey(result.items);
  ...
  const onDeleteAll = () => {
    ...
    const arrPromises = selectedIds.map((selectedId) => {
      const found = listPostExamSurvey.find((item) => item.id === selectedId);
      if (found?.id) {
    ...
    <div className={`page-content page__survey--form${isNoItem ? " bg-white" : ""}`}>
        ...
        <div className="action-backup">
          ...
          {showModalAdd && (
            <Fragment>
              ...
              />
              <h1 className="title-last">{dataPostExamSurvey ? "Chỉnh sửa" : "Thêm mới"}</h1>
            </Fragment>
          )}
        </div>
      ...
      <div className="card-box d-flex flex-column">
        <div className={`${showModalAdd ? "d-none" : ""}`}>
          ...
          />
          {!isLoading && listPostExamSurvey && listPostExamSurvey.length > 0 ? (
            <BoxTable
              name="khảo sát khách hàng"
              titles={titles}
              items={listPostExamSurvey}
              isPagination={true}
      ...
      <ViewResultSurvey onShow={showResultModal} data={dataPostExamSurvey} onHide={() => setShowResultModal(false)} />

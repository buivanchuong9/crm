...
export default function FilterModal(props: any) {
  ...
  useEffect(() => {
    if(params.isPriority){
        setTypeWork({value: params.isPriority, label: params.isPriority === 3 ? 'Công việc ưu tiên' : params.isPriority === 2 ? 'Công việc không ưu tiên' : 'Công việc gấp' })
    } else {
        setTypeWork(null);
    }
    ...
    if(params.status){
        setStatusWork({
            value: params.status,
            label: params.status === '0' ? 'Mới tiếp nhận'
                    : params.status === '1' ? 'Quá hạn'
                    : params.status === '2' ? 'Đã hoàn thành'
                    : params.status === '4' ? 'Tạm dừng'
                    : 'Đã hủy'
        })
    } else {
        setStatusWork(null);

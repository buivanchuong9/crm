...
import ModalNegotiationDetail from "./partials/ModalNegotiationDetail/ModalNegotiationDetail";
...
const handleUnfulfilled = (time: any) => {
...
const StatusLabel = ({ status, endTime }) => {
  const now = new Date();
    ...
    0:
      now > end
        ? {
            className: "__expire",
            icon: "ExpireWork",
            element: <>Quá hạn {handleUnfulfilled(endTime)}</>,
          }
        : {
            className: "__unfulfilled",
            icon: "NewWork",
            text: "Chưa tiếp nhận",
          },
    1:
  ...
  const { className, icon, text, element } = statusMap[status] || statusMap.default;

  return (
    <div className={className}>
      <Icon name={icon} />
      <span style={{ fontSize: 12, fontWeight: "500" }}>{text || element}</span>
    </div>
  );
};
...
const DetailWork = (props: IDetailWorkProps) => {
  ...
  const [isModalNegotiationDetail, setIsModalNegotiationDetail] = useState(false);
  ...
  const getListWorkPause = async (potId, processId, workId, data) => {
    ...
    const response = await WorkOrderService.listPause(params);
  ...
  const dataMappingArrayExternal = (item: any, index: number, type?: string) =>
    type === "ola"
      ? [
          ...
          item.workOrderId,
          item.workOrderName,
          item.contextData && JSON.parse(item.contextData) && JSON.parse(item.contextData).projectName,
          item.contextData && JSON.parse(item.contextData) && JSON.parse(item.contextData).prName,
          item.processName,
          item.responseTime,
          item.processTime,
          // item.createdTime,
          item.createdTime ? moment(item.createdTime, "DD/MM/YYYY HH:mm:ss").add(7, "hours").format("DD/MM/YYYY HH:mm:ss") : "",
          // item.firstSeenTime
          item.firstSeenTime ? moment(item.firstSeenTime, "DD/MM/YYYY HH:mm:ss").add(7, "hours").format("DD/MM/YYYY HH:mm:ss") : "",
          // item.processedTime,
          item.processedTime ? moment(item.processedTime, "DD/MM/YYYY HH:mm:ss").add(7, "hours").format("DD/MM/YYYY HH:mm:ss") : "",
          item.actualExecutionTime,
          item.lastProcessedTime,
          item.repeatNumber,
          // `${item.actualResponseDay || 0} ngày, ${item.actualResponseHour < 10 ? `0${item.actualResponseHour || 0}` : item.actualResponseHour}:${
          //   item.actualResponseMinute < 10 ? `0${item.actualResponseMinute}` : item.actualResponseMinute
          // }`,
          item.actualResponseTime,
          // `${item.actualExecutionDay || 0} ngày, ${item.actualExecutionHour < 10 ? `0${item.actualExecutionHour || 0}` : item.actualExecutionHour}:${
          //   item.actualExecutionMinute < 10 ? `0${item.actualExecutionMinute}` : item.actualExecutionMinute
          // }`,
          item.actualExecutionTime,
          item.violationResponseStatus === 1 ? "Vi phạm" : "",
          item.violationExecutionStatus === 1 ? "Vi phạm" : "",
          item.reason,
        ]
      : [
          index + 1,
  ...
  const exportExternal = useCallback(async (data, extension, type) => {
    const newArray = [data];
    const olaHistory = JSON.parse(data?.olaHistory || []);
    ...
    if (extension === "excel") {
      ExportExcel({
        ...
        header: titlesExcelOLA,
        formatExcel: formatExportOLA,
        data: olaHistory.map((item, idx) => dataMappingArrayExternal(item, idx, type)),
        info: { name },
      });
    }
  ...
  const handleUpdateStatusNegotiationWork = async (worId: number) => {
    const response = await WorkOrderService.getNegotiationWork(worId);
    if (response.code === 0) {
      if (response?.result?.negotiationBidderDetailId) {
        const body = {
          id: response?.result?.negotiationBidderDetailId,
        };
        const res = await WorkOrderService.completeNegotiationWork(body);
        if (res.code === 0) {
        } else {
          showToast(res.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
        }
      } else {
        showToast("Có lỗi xảy ra. Vui lòng thử lại sau (không tìm thấy thông tin NegotiationBidderDetailId)", "error");
      }
    } else {
      showToast(response.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const handleUpdateStatusWork = async (status, noToast?) => {
    ...
    if (response.code === 0) {
      if (status === 2) {
        ...
        if (data?.taskType === "open_negotiation") {
          handleUpdateStatusNegotiationWork(data?.id);
        }
      }
      if (status === 1) {
        if (!noToast) {
  ...
  const getNewTime = (listWorkPauseItem: (typeof listWorkPause)[number]) => {
    if (!listWorkPauseItem.data) return "";

    const newTime = JSON.parse(listWorkPauseItem.data)?.newTime;
    if (!newTime) return "";

    return `${moment(JSON.parse(listWorkPauseItem.data).newTime).format("DD/MM/YYYY - HH:mm")} - `;
  };

  const handleOpenTaskModal = (data: any) => {
    if (data?.status === -1) return showToast("Công việc đã được thu hồi", "warning");
    if (data?.scope !== "external") {
      ...
      if (data?.taskType === "open_negotiation") {
        if (data?.status === 0) {
          ...
          setIsModalNegotiationDetail(true);
        }
      }
    }
  };

  // const endOLAResponse = new Date(new Date(data?.startTime).getTime() + (((+valueOLAResponse?.day * 24 + +valueOLAResponse?.hour) * 60 + +valueOLAResponse?.minute) * 60 * 1000));
  ...
  return (
    <div className="detail__work--item">
                      ...
                      <div key={index} className="item_roadmap">
                        <div className="line_border" style={item.stepNumber < data?.workflowStep ? { backgroundColor: "#B7B8B9" } : {}}>
                          {data?.workflowId === item.id ? (
                            <div style={{ width: "100%", height: "0.5rem", backgroundColor: "#C94B1C", borderRadius: 10 }}></div>
                          ) : null}
                        </div>
                  ...
                  <div className="container-button">
                    ...
                    <div className="list-button">
                      ...
                      (data?.taskType === "assigned_task" ||
                        ...
                        data?.taskType === "open_negotiation") &&
                    ...
                    <div className="info__basic">
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.2rem" }}>
                        ...
                        <div className="show-status">
                          <StatusLabel status={data.status} endTime={data.endTime} />
                        </div>
                      </div>
                      <div className="info__basic--item">
                        ...
                        <div style={{ width: "100%", display: "flex", gap: "0 1rem" }}>
                          <div className="content-work" style={{ flex: 1 }}>
                            <span className="title">Nội dung công việc</span>
                            <div
                              className={`box_content ${data?.status === -1 ? "none-clickable" : ""}`.trim()}
                              onClick={() => {
                                handleOpenTaskModal(data);
                              }}
                            >
                              {data?.scope === "external" ? (
                          ...
                          <div style={{ display: "flex" }}>
                            <div>
                              <Icon
                                ...
                                style={{
                                  ...
                                  height: 20,
                                  fill:
                                    data.priorityLevel === 1 || data.priorityLevel === 2
                                      ? "var(--extra-color-30)"
                            ...
                            <div>
                              <span style={{ fontSize: 14, fontWeight: "00" }}>
                                {data.priorityLevel === 3 ? "Ưu tiên" : data.priorityLevel === 4 ? "Gấp" : "Không ưu tiên"}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        {valueOLAResponse || valueOLAProcess ? (
                          <div className="line_3">
                            ...
                            {/* <div className="count-down-OLA">
                              <div>
                                <span className="title">{data?.status === 0 ? `OLA phản hồi còn lại` : `OLA xử lý còn lại`}</span>
                                ...
                                <div className="item_step">
                                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                    <div style={{ width: "83%" }}>
                                      ...
                                      <span style={{ fontSize: 14, fontWeight: 500, color: "#2C2C2C" }}>Gia hạn thời gian nộp thầu</span>
                                    </div>
                                  </div>
      ...
      <ModalNegotiationDetail
        onShow={isModalNegotiationDetail}
        data={data}
        onHide={(reload) => {
          if (reload) {
            handleUpdateStatusWork(2);
          }
          setIsModalNegotiationDetail(false);
        }}
      />

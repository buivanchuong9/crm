...
export default function UpdatePeopleInvolved(props: any) {
  ...
  const getListRelatedPeople = async () => {
    ...
    if (response.code === 0) {
      ...
      const listAssign = result.assigners || [];

      setJobListAssign(listAssign);
      setJobAssign(result.manager || result.assigners[0]);
      ...
      <div className="update__people">
        {!isLoading ? (
          ...
          <div className="view-info">
            ...
            <Icon name="ChevronDoubleRight" style={{ fill: "#939394" }} />
            <div className="view-recipient">
              <div style={{ display: "flex" }}>
                <label className="label">Người nhận việc</label>
                {data?.scope === "external" && (data?.status === 0 || data?.status === 1) ? (
                  // && data?.managerId === dataEmployee?.id
                  <Tippy content="Đổi người nhận việc">
                    <div
                    ...
                    >
                      <Icon name="Pencil" style={{ width: 15, height: 15 }} />
                    </div>
                  </Tippy>
                ) : null}
              </div>

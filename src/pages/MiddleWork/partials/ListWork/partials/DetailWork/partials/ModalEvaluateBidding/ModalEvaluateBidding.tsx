...
import ModalBiddingProfile from "pages/TenderPackageList/DetailTenderPackage/ModalBiddingProfile/ModalBiddingProfile";
...
export default function ModalEvaluateBidding({ onShow, onHide, data }) {
  ...
  useEffect(() => {
    if (listBidding && listBidding.length > 0) {
      const listData = listBidding.map((item, index) => {
        return {
          value: index + 1,
          label: `Số ${index + 1}`,
        };
  ...
  const getListTenderPackage = async (paramsSearch: any, disableLoading?: boolean) => {
    ...
    if (response.code === 0) {
      ...
      if (result?.filter((el) => el.canUpdate === 1).length > 0) {
        setEvaluationType("finance");
      }
    ...
    item.canUpdate === 1 ? (
      <div>
        <SelectCustom
          ...
          special={true}
          options={listValue}
          value={item.position ? { value: item.position, label: `Số ${item.position}` } : null}
          onChange={(e) => {
            onSubmitEvaluate(e.value, item.documentDetailId);
            setListBidding((current) =>
              current.map((obj, idx) => {
                if (index === idx) {
                  return { ...obj, position: e.value };
                }
                return obj;
              })
            );
  ...
  const onSubmit = async (e) => {
    ...
    let response;
    if (evaluationType === "finance") {
      response = await TenderPackageService.submitReviewFinance(body);
    } else {
      response = await TenderPackageService.submitReview(body);
    }
  ...
  const onSubmitEvaluate = async (position, documentDetailId) => {
    const body = {
      position: position,
      documentDetailId,
    };

    const response = await TenderPackageService.updateBatchFinance(body);
    if (response.code === 0) {
      getListTenderPackage(params, true);
                ...
                {data?.status === 2 ? null : (
                  <div className="button">
                    <ButtonComponent
                      ...
                      callback={() => {
                        const positionIds = [];
                        for (const bidding of listBidding) {
                          if (positionIds.includes(bidding.position)) {
                            showToast("Thứ tự đánh giá không được trùng nhau, vui lòng kiểm tra lại", "error");
                            return;
                          }
                          positionIds.push(bidding.position);
                        }
                        setIsModalConfirm(true);
                      }}
                    />

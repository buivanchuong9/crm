...
import AddPatientRecordModal from "./partials/AddPatientRecordModal";
...
const AddManagementOpportunityModal = () => null;
import "swiper/css/grid";
import "swiper/css/navigation";
import "./PatientRecordList.scss";
...
export default function PatientRecordList() {
  ...
  document.title = `${showPageSendEmail ? "Gửi email" : showPageSendSMS ? "Gửi SMS" : activeTitleHeader === 1 ? "Hồ sơ Bệnh án (Thông tin BN, tiền sử bệnh)" : "Phân tích nguồn khách hàng"
    }`;
  ...
  const takeParamsUrl = useMemo(() => getSearchParameters(), [searchParams]);
  ...
  //Reset dữ liệu cuộc gọi khách hàng Tnex
  const resetDataCustomer = async (listIdCustomer) => {
    const body = {
      customerIds: listIdCustomer,
    };

    const response = await CustomerService.resetDataCustomer(body);

    if (response.code === 0) {
      setIsLoading(true);
      setTimeout(() => {
        showToast("Reset dữ liệu thành công", "success");
        getListCustomer(params, activeTitleHeader);
        setListIdChecked([]);
      }, 2000);
    } else {
    ...
    },
    {
      ...
      callback: () => {
        const contentDialog: IContentDialog = {
          ...
          isCentered: true,
          isLoading: true,
          title: <Fragment>Reset trạng thái cuộc gọi</Fragment>,
          message: (
            <Fragment>
              Toàn bộ lịch sử gọi Follow sẽ bị xóa và khách hàng sẽ được chuyển về người phụ trách Trần Minh Hiếu. Hành động không thể hoàn tác.
            </Fragment>
          ),
          cancelText: "Hủy",
          cancelAction: () => {
          ...
          defaultAction: () => {
            setShowDialog(false);
            setContentDialog(null);
            resetDataCustomer(listIdChecked);
          },
        };
        setContentDialog(contentDialog);
        setShowDialog(true);

...
import ModalAddProjectRealty from "./partials/ModalAddProjectRealty";

export default function ProjectRealtyList(props: any) {
  ...
  const getListProject = async (paramsSearch: any, disableLoading?: boolean) => {
    if(!disableLoading){
        setIsLoading(true);
    }
    
    const response = await ProjectRealtyService.list(paramsSearch, abortController.signal);
  ...
  const titleActions: ITitleActions = {
    actions: [
      ...
      // ] : [])
      permissions["LIST_PROJECT_ADD"] == 1 &&
        {
  ...
  const titles = ["STT", "Mã dự án", "Tên dự án", 'Địa điểm', 'Chủ đầu tư', ...(permissions["LIST_PROJECT_UPDATE"] == 1 ? ["Hoạt động"] : [])];
  ...
  const dataMappingArray = (item: any, index: number) => [
    ...
    item.address,
    item.investorName,
    ...(permissions["LIST_PROJECT_UPDATE"] == 1 ? [
  ...
  const actionsTable = (item: any): IAction[] => {
    ...
    permissions["LIST_PROJECT_DELETE"] == 1 &&
      {
  ...
  const onStatus = async (id, status) => {
    ...
    const response = await ProjectRealtyService.updateStatus(body);
  ...
  const onDelete = async (id: number) => {
    const response = await ProjectRealtyService.delete(id);
  ...
  const onDeleteAll = async () => {
    ...
    listIdChecked.map((item) => {
      const promise = new Promise((resolve, reject) => {
        ProjectRealtyService.delete(item).then((res) => resolve(res));
      });
  ...
  const showDialogConfirmDelete = (item?: IContractPipelineResponse) => {
    const contentDialog: IContentDialog = {
      ...
      message: (
        <Fragment>
          Bạn có chắc chắn muốn xóa {item ? "dự án " : `${listIdChecked.length} dự án đã chọn`}
  ...
  return (
    <div className="page-content page-project-realty-list card-box">
        <TitleAction title="Dự án" />
        <div className="d-flex flex-column">
            <HeaderFilter
              ...
              disableDeleteAll = {permissions["LIST_PROJECT_DELETE"] == 1 ? false : true}
            />
                ...
                <Fragment>
                    {isPermissions ? (
                        ...
                        <SystemNotification
                            description={
                                <span>
                                    Hiện tại chưa dự án nào. <br />
                                    Hãy thêm mới dự án đầu tiên nhé!
                                </span>
                            }
                            type="no-item"
                            titleButton=""

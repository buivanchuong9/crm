...
export default function ModalAddInvestor(props: any) {
  ...
  const listFieldBasic = useMemo(
    ...
    [formData?.values, dataCity, dataDistrict, dataSubdistrict]
  );
  ...
  const onSubmit = async (e) => {
    ...
    const response = await InvestorService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} chủ đâu tư thành công`, "success");
      handClearForm(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-investor"
        size="sm"
      >
        <form className="form-add-investor-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} chủ đầu tư`}
            toggle={() => {

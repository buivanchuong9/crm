...
import "./BoxTableSuppilerAccount.scss";
import Action from "components/boxTable/partials/action";
...
export default function BoxTableSuppilerAccount(props: BoxTableProps) {
  const {
    name,
    titles,
    actionType,
  ...
  const checkAll = (isChecked: boolean) => {
    if (isChecked) {
      const checkId = items.map((i) => {
        if(!i.active){
          return i.id;
        }
      }).filter(el => el);

      setListIdChecked && setListIdChecked(checkId || []);
      // setListIdChecked &&
      //   setListIdChecked(
      //     items.map((i) => {
      //       if(!i.status){
            ...
            <tr>
              {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                <th className="checkbox">
                  <Checkbox
                    indeterminate={listIdChecked?.length > 0 && listIdChecked?.length < items.filter(el => !el.active).length}
                    checked={listIdChecked?.length > 0 && listIdChecked?.length === items.filter(el => !el.active).length}
                  ...
                  >
                    {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                      <td className="checkbox" onClick={(e) => e.stopPropagation()}>
                        <Checkbox checked={isChecked} onChange={(e) => checkOne(item.id, e.target.checked)} disabled={!item?.raw?.active ? false : true}/>
                      </td>
                    )}

...
export default function NotificationList(props: any) {
  ...
  const moveHandleWork = (workId) => {
    navigate('/bpm/task_assignment', {state: {viewDetail: true, workId: workId}})
  }

  const moveHandleNotification = (item) => {
    if (item.type === 2 && item.content && isJsonString(item.content)) {
      const content = JSON.parse(item.content);
      if (content.packageId) {
        navigate("/bpm/bid_management", { state: { viewDetail: true, packageId: content.packageId } });
        return;
      }
    }
    moveHandleWork(item.workId);
    return;
  ...
  const renderNotifyExpire = (item: any) => {
    let contentObj = null;
    ...
    if (item.content && isJsonString(item.content)) {
      try {
        ...
        isShowContent = true;
        if (contentObj?.type === 'EXTEND_REQUEST') {
          title = "Yêu cầu gia hạn đã được gửi tới";
          const parts = [
            contentObj?.organizationName,
            contentObj?.packageName,
    ...
    return (
      <>
        <Icon name='NotifyExpire'/>
        <div className="body-notification">
          <div className="title-notification">
            <div className="box-title">
              <span className="title">{title}</span>
            </div>
            {item.unread === 1 ? <div className="icon-red"/> : null}
          </div>
          {/* { isShowContent ? 
          ...
          <div className="footer-notification">
            {item.projectName ? 
              <div className="box-project">
                <span className="name-project">{item.projectName}</span>
              </div>
            : null}
            <span className="time">{item.timeReceived ? moment(item.timeReceived).format('DD/MM/YYYY - HH:mm') : ''}</span>
          </div>
        </div>
      </>
    );
                  ...
                  item.type === 3 ? 
                    ...
                    >
                      ...
                      <div className="body-notification">
                        <div className="title-notification">
                          <div className="box-title">
                            <span className="title">Thông báo hệ thống</span>
                          </div>
                          <div className="icon-red"/>
                        </div>
                        <div className="content-notification">
                    ...
                    <div 
                      ...
                      onClick={() => {
                        onUnread(item.id);
                        moveHandleNotification(item);
                      }}
                    >
                      {renderNotifyExpire(item)}
                    </div>
                  )
                : (
                  item.type === 3 ? 
          ...
          <div className="notify-type">
            <CheckboxList
              ...
              options={[
                {
                  value: '1',
                  label: "Công việc theo quy trình",
                },
                {
                  value: '2',

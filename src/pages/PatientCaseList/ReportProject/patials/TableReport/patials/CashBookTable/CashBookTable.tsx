...
export default function CashBookTable({ dataProjectReport }) {
  ...
  const [params, setParams] = useState<ICashbookFilterRequest>({
    ...
    fromTime: "",
    type: tab.type,
    projectId: dataProjectReport?.id || 0,
  });
  ...
  const getListCashBook = async (paramsSearch: ICashbookFilterRequest) => {
    ...
    if (dataProjectReport) {
      paramsSearch.projectId = dataProjectReport.id;
    }
  ...
  const getListCashBookReport = async (paramsSearch: ICashbookFilterRequest) => {
    ...
    if (response.code === 0) {
      ...
      const arr = (result.items || []).map((item, idx) => {
        if (item.type == 1) {
          total += +item.amount;
        } else {
          total += -+item.amount;
  ...
  return (
    <div className={`page-content cashbook-report cashbook${isNoItem ? " bg-white" : ""}`}>
      ...
      {tab.type == 1 ? componentCashbook() : componentReport()}

...
export default function ModalAddUnit(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} đơn vị tính thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        size="sm"
      >
        <form className="form-add-unit-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} đơn vị tính`}
            toggle={() => {

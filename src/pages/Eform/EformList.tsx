...
export default function EformList(props: any) {
  ...
  const onDeleteAll = async () => {
    ...
    listIdChecked.map((item) => {
      const promise = new Promise((resolve, reject) => {
        ContractEformService.delete(item).then((res) => resolve(res));
      });
    ...
    Promise.all(arrayPromise).then((result) => {
      if (result.length > 0) {
        showToast("Xoá biểu mẫu thành công", "success");
        getListContractEform(params);
        setListIdChecked([]);
      } else {
  ...
  return (
    <div className={`page-content page-eform${isNoItem ? " bg-white" : ""}`}>

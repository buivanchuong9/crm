...
// import { IColumnGrid } from "pages/Grid";
import { convertToFileName } from "reborn-util";
import GridService from "services/GridService";
import AddFile from "pages/BPM/GridForm/partials/ModalImport/partials/AddFile";
...
export default function ModalImport(props: any) {
  ...
  const [customType, setCustomerType] = useState("0");
  ...
  //     ws,
  //     [
  //       // ["Tên khách hàng", "Tuổi", "Địa chỉ"]
  //       listColumn.map((item) => item.name),
  //     ],
  //     { origin: "A1" }
  //   );

  //   // Thêm dòng thứ 2 với các key
  //   XLSX.utils.sheet_add_aoa(
  //     ws,
  //     [
  //       // ["name", "age", "address"]
  //       listColumn.map((item) => item.key),
  //     ],
  //     { origin: "A2" }
  //   );
  ...
  //   saveAs(blob, "File_mau_" + code_name + ".xlsx");
  // };
    ...
    reader.onload = (event) => {
      ...
      jsonData = jsonData.map((row: any, index: number) => {
        ...
        for (const key in row) {
          if (invalid) return;
          if (index > 1 && dataRow[index - 1] && dataRow[index - 1].type !== "title") {
            const gridRow = dataRow[index - 1];
            const cell = gridRow.find((c) => c.name === key);
          ...
          if (typeof row[key] === "number") {
            newRow[key] = parseFloat(row[key].toFixed(4)); // Giới hạn 4 chữ số thập phân
          }
        }
      ...
      setNumberLine(jsonData.length - 3);
    };
  ...
  const handConfirmUploadFile = (data, parmas) => {
    if (!data) return;
    setIsSubmit(true);
    // uploadDocumentFormData(data, onSuccess, onError, onProgress, type, +parmas);
    onHide(true);
  };
  ...
  const onSuccess = async (data) => {
    ...
    setTimeout(() => {
      ...
      setStep({ ...step, stepOne: false, stepTwo: false, stepThree: true });
    }, 800);
  };
  ...
  const getDataRowsUpload = async (requestId) => {
    ...
    if (responseDataImport.code === 0) {
      if (responseDataImport?.result?.key == 1) {
        lookupDone = true;
        setDataImport(responseDataImport?.result?.value);
        setStep({ ...step, stepOne: true, stepTwo: false, stepThree: false });
        onHide(true);
      }
    } else {
      lookupDone = true;
      showToast(responseDataImport.message ?? "Có lỗi xảy ra. Vui lòng thử lại sau !", "error");
    }
    return lookupDone;
  };
  ...
  const handDownloadFileError = async () => {
    ...
    let response = null;
    if (type === "customer") {
      response = await CustomerService.downloadFile(infoFile.uploadId);
    } else {
      response = await PartnerService.downloadFile(infoFile.uploadId);
    }
    if (response) {
      ...
      link.download = `file_error_customer.xlsx`;
      link.click();
    } else {
  ...
  const createAndDownloadExcel = async () => {
    ...
    //   { "Tên khách hàng": "Lê Văn C", Tuổi: 28, "Địa chỉ": "Đà Nẵng" },
    // ];
    const listColumnExport = [{ key: "stt", name: "STT", type: "number" }, ...listColumn];

    // Dữ liệu mẫu có sẵn STT = 1
    const data = [
      {
        STT: 1,
        ...listColumn.reduce((acc, col) => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      listColumnExport.map((item) => item.name), // Dòng tiêu đề
      listColumnExport.map((item) => item.key), // Dòng thứ 2 với các key
      listColumnExport.map((item) => (item.type == "number" ? "Number" : "Nvarchar")), // Dòng thứ 3 với các kiểu dữ liệu
      listColumnExport.map((item) => (item ? "" : "")), // Dòng thứ 4 để lookup BE
      // listColumn.map((item) => (item ? "Lookup FE" : "Lookup FE")), // Dòng thứ 5 để lookup FE
    ]);
  ...
  const handDownloadFileSample = (file) => {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        link.download = "Mau_nhap_khach_hang.xlsx";
  ...
  const initalFieldChoose = [
    { name: "Tên khách hàng(*)", fieldName: "name" },
    { name: "Số điện thoại(*)", fieldName: "phone" },
    { name: "Chi nhánh(*)", fieldName: "branchName" },
  ];
  ...
  const handGetExAttributes = async (customType) => {
    const params = {
      custType: +customType,
    };
    ...
    if (type === "customer") {
      response = await CustomerService.exAttributes(params);
    } else {
      response = await PartnerService.exAttributes();
    }
  ...
  useEffect(() => {
    if (onShow && customType) {
      // handGetExAttributes(customType);
      setAddFile("");
    }
  }, [onShow, customType]);
  ...
  const handSubmitDataTest = async (count: number, customType, type) => {
    if (!count) return;
    ...
    if (type === "customer") {
      response = await CustomerService.numberFieldCustomer(body, param);
    } else {
      response = await PartnerService.numberFieldPartner(body);
    }
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = type === "customer" ? `Mau_nhap_khach_hang.xlsx` : `Mau_nhap_doi_tac.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const actions = useMemo<IActionModal>(
      ...
      actions_right: {
        buttons: [
...

          {
            ...
            type: "button",
            callback: () => {
              handConfirmUploadFile(addFile, customType);
              // if (step.stepOne) {
              //   if (addFile !== prevAddFile) {
              //     handConfirmUploadFile(addFile, customType);
              //   } else {
    ...
    [isSubmit, addFile, step, formData, customType]
  );
      ...
      >
        {caclData ? (
          <div className="import-loading">
            <div>Đang tải dữ liệu, vui lòng chờ vài phút...</div>
            <Icon name="Refresh" />
            <div>{"Hoàn thành " + lineSuccess + " trên tổng số " + numberLine + " bản ghi"}</div>
          </div>
        ) : (
          <form className="form__import--backup" onSubmit={(e) => onSubmit(e)}>
            ...
            <ModalBody>
              <div className="box__ui--import">
                {/* // Gọi hàm khi cần thiết, ví dụ khi nhấn nút */}
                {/* <button onClick={createAndDownloadExcel}>Tải xuống danh sách khách hàng</button> */}
                {/* {step.stepOne ? ( */}
                {true ? (
                  <div className="wrapper__step--one">
                    ...
                    {exampleFile && (
                      <div className="file__example">
                        <h4>
                          ...
                          <span className="download__file--sample" onClick={() => createAndDownloadExcel()}>
                            Excel file
                          </span>
                        </h4>
                      </div>
                    ...
                    <div className="download__sample--data" style={type === "partner" ? { marginTop: "2rem" } : {}}>
                      {/* <h4 className="name__sample" style={{ marginTop: "2rem" }}>
                        Tải dữ liệu mẫu:
                      </h4> */}
                      ...
                      <div className="info__sample--data">
                        ...
                        <div
                          className={`btn__submit--data ${hasSubmitDataTest ? "dis__submit--data" : ""}`}
                          ref={refOptionContainer}
                          style={{ marginTop: "1.5rem" }}
                        >
                          {/* <div className="btn__confirm" onClick={() => !hasSubmitDataTest && handSubmitDataTest(countDataTest, customType, type)}> */}
                          ...
                          >
                            ...
                            {hasSubmitDataTest ? "Đang tải" : "Tải xuống file mẫu"}
                          </div> */}
                            ...
                            <div className="info__field--customer" ref={refOption}>
                                  ...
                                  return (
                                    <Fragment key={idx}>
                                      <Checkbox
                                        ...
                                        disabled={["name", "phone", "branchName"].includes(item.fieldName)}
                                        label={item.name}
                              ...
                              <div className="confirm__field--submit">
                                <Button
                                  ...
                                  onClick={(e) => {
                                    e.preventDefault();
                                    // handImportFielDownload(countDataTest, lstFieldChoose, lstFieldChooseChildren, customType, type);
                                  }}
                                >
                  ...
                  <div className="wrapper__step--two">
                    <div className="form-group">
                      <Input
                        ...
                        fill={true}
                        required={true}
                        placeholder="Nhập tên nguồn dữ liệu khách hàng"

...
export default function ImportModal(props: IImportModalProps) {
  ...
  const handConfirmUploadFile = (data, parmas) => {
    if (!data) return;
    setIsSubmit(true);
    uploadDocumentFormData(data, onSuccess, onError, onProgress, 'contact', +parmas);
  };
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ContactService.autoProcess(body);
  ...
  const handDownloadFileError = async () => {
    ...
    const response = await ContactService.downloadFile(infoFile.uploadId);
  ...
  const handGetExAttributes = async () => {
    ...
    const response = await ContactService.exAttributes(params);
  ...
  const handSubmitDataTest = async (count: number, typeContract) => {
    ...
    const response = await ContactService.numberFieldContact(body, param);
    ...
    if (response) {
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `Mau_nhap_nguoi_lien_he.xlsx`;
      link.click();
    } else {
      showToast("Có lỗi xảy ra. Vui lòng thử lại sau", "error");
    }
  ...
  const actions = useMemo<IActionModal>(
    ...
    [isSubmit, addFile, step, formData]
  );
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal__import--contact"
        size={`${infoFile?.data ? "xl" : "md"}`}
      >
        <form className="form__import--contact" onSubmit={(e) => onSubmit(e)}>

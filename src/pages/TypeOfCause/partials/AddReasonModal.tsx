...
export default function AddReasonModal(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ReasonListBpmService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} nguyên nhân thành công`, "success");
      onHide(true);
    } else {
    ...
    <Fragment>
      ...
      >
        <form className="form-add-reason-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} nguyên nhân`}
            toggle={() => {

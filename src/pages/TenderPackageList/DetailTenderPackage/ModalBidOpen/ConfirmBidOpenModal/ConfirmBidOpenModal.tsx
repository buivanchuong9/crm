...
export default function ConfirmBidOpenModal(props: any) {
  ...
  const { onShow, onHide, data, tableTechnicalEmployees, tableFinanceEmployees, viewFinance } = props;
  ...
  const onSubmit = async (e) => {
    ...
    const techniques = tableTechnicalEmployees.map(item => item.employeeId);
    const finances = tableFinanceEmployees.map(item => item.employeeId);
    ...
    const body = {
      ...
      packageId: data?.id,
      potId: data?.potId,
      attachmentFinances: viewFinance
    };    
    
    const resutlResponseTime = await TenderPackageService.openBidding(body);
    if (resutlResponseTime.code === 0) {
      onHide(true);
      showToast(`Xác nhận mở thầu thành công`, "success");
      setIsSubmit(false);
      handClearForm();
    } else {

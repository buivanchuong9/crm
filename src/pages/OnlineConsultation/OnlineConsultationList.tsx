...
export default function OnlineConsultationList() {
  document.title = "Chăm sóc khách hàng";

  const [searchParams, setSearchParams] = useSearchParams();
  const [listOnlineConsultation, setListOnlineConsultation] = useState<ICustomerResponse[]>([]);
  const [listIdChecked, setListIdChecked] = useState<number[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [isNoItem, setIsNoItem] = useState<boolean>(false);
  const [showModalAdd, setShowModalAdd] = useState<boolean>(false);
  const [dataOnlineConsultation, setDataOnlineConsultation] = useState<ICustomerResponse>(null);
  ...
  const getListCustomer = async (paramsSearch: ICustomerSchedulerFilterRequest) => {
    ...
    if (response.code === 0) {
      ...
      setListOnlineConsultation(result.items);
    ...
    return [
      {
        ...
        callback: () => {
          setDataOnlineConsultation(item);
  ...
  return (
    <div className={`page-content page-customer${isNoItem ? " bg-white" : ""}`}>
      ...
      <div className="card-box d-flex flex-column">
        ...
        />
        {!isLoading && listOnlineConsultation && listOnlineConsultation.length > 0 ? (
          <BoxTable
            name="Khách hàng"
            titles={titles}
            items={listOnlineConsultation}
            isPagination={true}

...
import SelectLookup from "pages/AIImageAnalysis/GridFormSetting/partials/SelectLookup/SelectLookup";
...
export default function TableParticipantRule({ dataNode, processId, childProcessId, dataConfigAdvance, setDataConfigAdvanceEdit }) {
  ...
  useEffect(() => {
    if (listColumn && listColumn?.length > 1) {
      setEditColumn(
        listColumn.map((item) => {
          return {
      ...
      if (type === "binding") {
        ...
        const updatedDataNew = updatedData.map((row, rIdx) => {
          ...
          return row.map((field, fIdx) => {
            ...
            } else if (field.key == "jteId") {
              if (updatedData[rowIndex][fieldIndex].key == "departmentId" && rIdx == rowIndex) {
                return {
                  ...field,
                  value: "",
                  options: value?.options_jte || [],
                };
              }
            }
            return field;
  ...
  return (
    <div className="table-participant-rule">

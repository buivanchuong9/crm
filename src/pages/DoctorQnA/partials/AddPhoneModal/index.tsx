import React, { useEffect, useMemo, useState } from "react"; // HMR force update
import { IActionModal } from "model/OtherModel";
import { IAddPhoneModalProps } from "model/callCenter/PropsModel";
import { ITransferCallModel } from "model/callCenter/DoctorQnARequestModel";
...
import DoctorQnAService from "services/DoctorQnAService";
import "./index.scss";
import HistoryModal from "../HistoryModal/HistoryModal";

export default function AddPhoneModal(props: IAddPhoneModalProps) {
  const { onShow, dataCustomer, onHide } = props;
  ...
  const getPhoneCallCustomer = async (id: number) => {
    const response = await DoctorQnAService.makeCall(id);
  ...
  const handDisconnect = async (id: number) => {
    onHide();
    const response = await DoctorQnAService.hangupCall(id);
  ...
  const handTransferCall = async (idCustomer, sipEmployee) => {
    ...
    const response = await DoctorQnAService.transferCall(body);

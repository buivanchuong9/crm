...
import { IDoctorQnAResponseModel } from "model/callCenter/DoctorQnAResponseModel";
import { ICallHistoryListFilterRequest } from "model/callCenter/DoctorQnARequestModel";
...
export default function CallHistory(props: ICallHistoryProps) {
  ...
  const [listHistory, setListHistory] = useState<IDoctorQnAResponseModel[]>([]);
  ...
  const getListHistory = async (paramsSearch: ICallHistoryListFilterRequest) => {
    ...
    const response = await DoctorQnAService.callHistoryList(paramsSearch);
  ...
  const dataMappingArray = (item: IDoctorQnAResponseModel, index: number) => [

...
import "./ModalAddProjectCatalog.scss";
...
export default function ModalAddProjectCatalog(props: any) {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await ProjectCatalogService.update(body);

    if (response.code === 0) {
      showToast(`${data ? "Cập nhật" : "Thêm mới"} loại dự án thành công`, "success");
      onHide(true);
    } else {
  ...
  return (
    <Fragment>
      <Modal
        ...
        className="modal-add-project-catalog"
        size="sm"
      >
        <form className="form-add-project-catalog-group" onSubmit={(e) => onSubmit(e)}>
          <ModalHeader
            title={`${data ? "Chỉnh sửa" : "Thêm mới"} loại dự án`}
            toggle={() => {

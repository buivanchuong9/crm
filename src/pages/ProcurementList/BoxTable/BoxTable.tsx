            ...
            <tr>
              {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                <th className="checkbox">
                  <Checkbox
                    indeterminate={listIdChecked?.length > 0 && listIdChecked?.length < items.filter(el => !el.status).length}
                    checked={listIdChecked?.length > 0 && listIdChecked?.length === items.filter(el => !el.status).length}
                  ...
                  >
                    {isBulkAction && bulkActionItems && bulkActionItems?.length > 0 && listIdChecked && setListIdChecked && (
                      <td className="checkbox" onClick={(e) => e.stopPropagation()}>
                        <Checkbox checked={isChecked} onChange={(e) => checkOne(item.id, e.target.checked)} disabled={!item?.raw?.status ? false : true}/>
                      </td>
                    )}

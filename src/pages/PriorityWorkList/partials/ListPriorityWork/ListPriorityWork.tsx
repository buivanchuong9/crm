...
export default function ListPriorityWork(props: IListWorkProps) {
  ...
  const [listSaveSearch] = useState<ISaveSearch[]>([
    {
      key: "all",
      name: "Danh sách công việc ưu tiên",
      is_active: true,
    },
  ]);
  ...
  const titles = ["STT", "Tên công việc", "", "Dự án", "Thời gian", "Tên gói thầu mua sắm", "Người nhận việc", "Trạng thái"];
  ...
  const dataMappingArray = (item: any, index: number, type?: string) =>
    type !== "export"
      ? [
          ...
          <div style={{ width: "13rem" }}>{item.extendedData && JSON.parse(item.extendedData) ? JSON.parse(item.extendedData).projectName : ""}</div>,
  ...
  return (
    <div className={`page-content page-priority-work-list${isNoItem ? " bg-white" : ""}`}>
          ...
          <div className="action-header">
            ...
            <div className="line__height--work">
              <div className="container-button-header">
                <div className={"style-list-button-active"}>
                  <Icon name="ListData" />

...
export default function ImportModal(props: IImportModalProps) {
  const { onShow, onHide, name, code, type = "customer" } = props;
  ...
  const [lstTypeCustomer] = useState([
    {
      value: "0",
  ...
  const onSubmit = async (e) => {
    ...
    if (type === "customer") {
      response = await CustomerService.autoProcess(body);
    } else {
      response = await PartnerService.autoProcess(body);
    }
